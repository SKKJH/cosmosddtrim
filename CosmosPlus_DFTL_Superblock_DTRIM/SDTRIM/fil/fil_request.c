...
int FIL_IsIdle(void)
{
//For chip utilization
/*#ifndef WIN32
	static unsigned long long  next_time_to_print = 0;
	static unsigned long long next_time_to_test;
	static UINT32 test_count;
	static UINT32 total_ways;

	if (next_time_to_print == 0)
	{
		test_count = 0;
		total_ways = 0;
		next_time_to_print = OSAL_GetCurrentTickUS() + 5000000;
		next_time_to_test = OSAL_GetCurrentTickUS() + 100000;
	}

	if (OSAL_GetCurrentTickUS() > next_time_to_test)
	{
		test_count++;
		next_time_to_test = OSAL_GetCurrentTickUS() + 100000;
		for (UINT32 channel = 0; channel < USER_CHANNELS; channel++)
		{
			...
			{
				//For chip utilization
				if (g_pstFTLRequestInfo->anIssuedCount[channel][way] + g_pstFTLRequestInfo->anWaitCount[channel][way] != 0)
				{
					total_ways++;
				}
			}
	...
	if (OSAL_GetCurrentTickUS() > next_time_to_print)
	{
		UINT32 time_period = OSAL_GetCurrentTickUS() - next_time_to_print;
		next_time_to_print = OSAL_GetCurrentTickUS() + 5000000;
		xil_printf("Utilization - %u  - %u / %u / %u  \r\n", time_period, total_ways, test_count, ((total_ways / test_count) * 100) / 16);
...
BOOL _FIL_IsEmptyWay(int channel, int way)
{
	if (g_pstFTLRequestInfo->anIssuedCount[channel][way] + g_pstFTLRequestInfo->anWaitCount[channel][way] != 0)
	{
		return FALSE;
...
static void _ProcessIssuedQ(int nCh, int nWay)
{
	...
	/*if (FIL_IsBad(nCh, nWay, pstRequest->stAddr.nBlock)) {
		DEBUG_ASSERT(0);
	}*/
	//OSAL_SleepUS(2);
	/*UINT32 temp = 0;
	for (int iter = 0; iter < 200; iter++) // mendatory sleep
		temp++;*/
