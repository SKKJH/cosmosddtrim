...
SUPER_GC_MGR::CheckAndStartGC()
{
	...
	if ((TrimPending_IsOn()) && (!g_trim_going))
	{
		...
		UINT32 SegmentCnt = FindTopKTrimSegments(32);
		CheckPendingTrim();
	...
	if (CandVBN != INVALID_VBN)
	{
		...
		if (CandVBN > 19)
		{
		    auto get_vcnt = [&](int ch, int wy) -> UINT32 {
		        ...
		        if (mgr) {
		            VBINFO *vb = mgr->GetVBInfo(CandVBN);
		            if (vb) {
		                bool is_free = false;
		                is_free = vb->IsFree();
		                if (!is_free) {
...
//		    xil_printf("[GC START] VBN:%u(%d USED), VCNT:[%u, %u, %u, %u], FREE:%u\r\n",
//		               CandVBN,
//					   DFTL_GLOBAL::GetSBInfoMgr()->m_pastSBInfo[CandVBN].m_nUSED,
//		               get_vcnt(0,0),
//		               get_vcnt(0,1),
//		               get_vcnt(1,0),
//		               get_vcnt(1,1),
