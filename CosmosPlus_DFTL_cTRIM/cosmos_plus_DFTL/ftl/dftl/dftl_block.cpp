...
// 상태 라인 프린트: 기존 스타일과 유사하게 [A][M][F] 태그 + invalid/total
static inline void _print_vb_line_state(const VBINFO* vb, UINT32 ch, UINT32 wy, UINT32 vppb)
{
    ...
    PRINTF("    VBN=%u inv=%u/%u ch=%u wy=%u [%s%s%s]\n\r",
           vb->m_nVBN, inv, vppb, ch, wy,
           vb->IsActive() ? "A" : "",
           vb->IsMeta()   ? " M" : "",
           vb->IsFree()   ? " F" : "");
}

// 채널/웨이 하나에 대해 VBN=0..VBlockCount-1 순회 출력
VOID BLOCK_MGR::DebugPrintAllByVBN_CW(UINT32 channel, UINT32 way) const
{
    ...
    PRINTF("-- SUMMARY: free=%u (used approx=%u)\n\r",
           m_nFreeBlocks[channel][way],
           vblk_cnt - m_nFreeBlocks[channel][way]); // 간단 요약
}

// 전 채널/웨이에 대해 전수 출력
VOID BLOCK_MGR::DebugPrintAllByVBN() const
{
    for (UINT32 ch = 0; ch < USER_CHANNELS; ++ch) {
        for (UINT32 wy = 0; wy < USER_WAYS; ++wy) {
            DebugPrintAllByVBN_CW(ch, wy);
        }
    }
}
