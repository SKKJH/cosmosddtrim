...
VOID DFTL_GLOBAL::WritePtr_GetAndAdvance()
{
    m_wp_page++;
    if (m_wp_page == LPN_PER_PHYSICAL_PAGE) { // 16KB(=4×4KB) 채울 때마다
        m_wp_page = 0;     // 다음 물리 페이지로 넘김
        m_wp_ch++;
        if (m_wp_ch == USER_CHANNELS) {
            m_wp_ch = 0;
            m_wp_wy++;
            if (m_wp_wy == USER_WAYS) {
                m_wp_wy = 0;
            }
        }
    }
...
VOID DFTL_GLOBAL::DebugBlockPrint(VOID)
{
    GetUserBlockMgr()->DebugPrintAllByVBN();
}
