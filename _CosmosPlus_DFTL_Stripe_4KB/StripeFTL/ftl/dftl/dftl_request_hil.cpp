...
//		   xil_printf("IS NOT WRITABLE (GC RUNNING)\r\n");
      return FALSE;
   }
   ...
   {
//	   if (DFTL_GLOBAL::GetInstance()->m_bEnable > 1)
//		   xil_printf("IS NOT WRITABLE (Not Enough Free Buffer)\r\n");
      return FALSE;
   }

//   if (DFTL_GLOBAL::GetInstance()->m_bDSMEnable > 1)
//	   xil_printf("IS WRITABLE \r\n");
...
////   xil_printf("[0] m_nLPN:%u, m_nIssued_Count:%u, last_lpn:%u\r\n", m_nLPN, m_nIssued_Count, last_lpn);
//
//   for (int i = m_nIssued_Count; i < m_nLPNCount; i++) {
//      INT32 channel, way;
...
//            break;
//         }
////    	 xil_printf("[1] LPN:%u, LPN COUNT:%u [%u/%u]\r\n", m_nLPN + m_nIssued_Count, lpn_count, last_ch, last_wy);
...
//
//   if (pstRequest_per_way) {
////  	 xil_printf("[2] LPN:%u, LPN COUNT:%u [%u/%u]\r\n", m_nLPN + m_nIssued_Count, lpn_count, last_ch, last_wy);
//      pstRequest_per_way->Initialize_per_way(HIL_REQUEST_WRITE_WAIT, NVME_CMD_OPCODE_WRITE,
//         m_nLPN + m_nIssued_Count, m_nHostCmdSlotTag, lpn_count, this);
//
//      pstHILRequestInfo->AddToWaitQ_per_way(pstRequest_per_way, last_ch, last_wy);
...
//         lpn_count++;
//      } else {
//         int tgt_ch = DFTL_GLOBAL::GetInstance()->m_wp_ch;
//         int tgt_wy = DFTL_GLOBAL::GetInstance()->m_wp_wy;
//
//         pstRequest_per_way = pstHILRequestInfo->AllocateRequest_per_way();
//         if (!pstRequest_per_way) {
//            break;
...
//            IncIssued_count();
//         }
//         DFTL_GLOBAL::GetInstance()->WritePtr_GetAndAdvance();
//
//         lpn_count = 1;
//         last_lpn  = lpn;
//      }
...
//      GoToNextStatus();
//
//      REQUEST_MGR*      pstRequestMgr2     = DFTL_GLOBAL::GetRequestMgr();
//      HIL_REQUEST_INFO* pstHILRequestInfo2 = pstRequestMgr2->GetHILRequestInfo();
//
...
HIL_REQUEST::_ProcessWrite_Wait(VOID)
{
   ...
   while (m_nIssued_Count < m_nLPNCount) {
      ...
      pstRequest_per_way->Initialize_per_way(
         HIL_REQUEST_WRITE_WAIT, NVME_CMD_OPCODE_WRITE,
         m_nLPN + m_nIssued_Count, m_nHostCmdSlotTag, /*count=*/1, this);
      pstHILRequestInfo->AddToWaitQ_per_way(pstRequest_per_way, tgt_ch, tgt_wy);
            ...
            if (ReadCacheBuf->readtype == 1)
            {
               if (((UINT32*)ReadCacheBuf->m_pSpareBuf)[LPN_OFFSET_FROM_VPPN(nVPPN)] != nLPN)
               {
                  ...
                  else
                  {
                     ...
                     nBufferingVPPN = pstBufferingLPN->ReadLPN(nLPN, ReadCacheBuf, channel, way);
...
skip_readbuf:
         ...
         nBufferingVPPN = pstBufferingLPN->ReadLPN(nLPN, pBufEntry[i], channel, way);
...
HIL_REQUEST_PER_WAY::_ProcessWrite_Wait(VOID)
{
   ...
   do
   {
      ...
      bSuccess = pstActiveBlock->Write(this, IOTYPE_HOST, IOTYPE_HOST);
