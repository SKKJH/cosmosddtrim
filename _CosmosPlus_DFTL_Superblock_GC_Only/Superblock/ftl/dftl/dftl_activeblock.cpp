...
VOID
ACTIVE_BLOCK::_IssueProgram(IOTYPE GC_TYPE)
{
	...
	stProgramReqID.stProgram.nActiveBlockIndex = DFTL_GLOBAL::GetActiveBlockMgr(stProgramReqID.channel, stProgramReqID.way)->GetActiveBlockIndex(m_eBlockType);
...
*/
BOOL
ACTIVE_BLOCK::Write(VOID* pstRequest, IOTYPE eRequestType, IOTYPE GC_TYPE)
{
	...
	if (m_eBlockType == IOTYPE_HOST)
	{
		...
		// host request only
		BUFFERING_LPN* pstBufferingLPN = DFTL_GLOBAL::GetActiveBlockBufferingLPN(pstHILRequest->GetChannel(), pstHILRequest->GetWay());
	...
	else if (m_eBlockType == IOTYPE_GC)
	{
		...
		BUFFERING_LPN* pstBufferingLPN = DFTL_GLOBAL::GetActiveBlockBufferingLPN(pstGCRequest->GetChannel(), pstGCRequest->GetWay());
	...
	else if (m_eBlockType == IOTYPE_META)
	{
		...
		BUFFERING_LPN* pstBufferingLPN = DFTL_GLOBAL::GetMetaActiveBlockBufferingLPN(channel, way);
...
ACTIVE_BLOCK::ProgramDone(INT32 nBufferingIndex)
{
	...
	{
		pstBufferingLPN = DFTL_GLOBAL::GetActiveBlockBufferingLPN(channel, way);
	}
#if (SUPPORT_META_DEMAND_LOADING == 1)
	else
	{
		pstBufferingLPN = DFTL_GLOBAL::GetMetaActiveBlockBufferingLPN(channel, way);
	}
#else
	else
	{
...
*/
UINT32
BUFFERING_LPN::ReadLPN(UINT32 nLPN, BUFFER_ENTRY* pstBufferEntry, UINT32 channel, UINT32 way)
{
	UINT16	nIndex = BUFFERING_LPN::GET_BUCKET_INDEX(nLPN);
//	UINT32 channel, way;
...
//	way = get_way_from_lpn(nLPN);
	// Get Active Block
	ACTIVE_BLOCK_MGR*	pstActiveBlockMgr = DFTL_GLOBAL::GetActiveBlockMgr(channel, way);
...
ACTIVE_BLOCK_MGR::_GoToNextActiveBlock(IOTYPE eIOType)
{
	...
	// Allocate New Block
	UINT32 nVBN = pstBlockMgr->Allocate(m_nchannel, m_nway, bUser, bGC, bMeta, 1);
...
ACTIVE_BLOCK_MGR::GetActiveBlock(IOTYPE eIOType)
{
	...
	if (pstActiveBlock->m_nCurVPPN == INVALID_PPN)
	{
		...
		{
			if (DFTL_GLOBAL::GetInstance()->m_GCActiveBlockAllocCnt == (USER_CHANNELS * USER_WAYS))
			{
				int check = 0;
				for (int i=0; i<USER_CHANNELS; i++)
				{
			...
			DFTL_GLOBAL::GetInstance()->m_GCActiveBlockAllocCnt -= 1;
			if (DFTL_GLOBAL::GetInstance()->m_GCActiveBlockAllocCnt == 0)
				DFTL_GLOBAL::GetInstance()->m_GCActiveBlockAllocCnt = USER_CHANNELS * USER_WAYS;
		}
