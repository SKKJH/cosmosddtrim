...
typedef enum
{
	...
	NVME_CMD_OPCODE_DSM = IO_NVM_DATASET_MANAGEMENT,
	NVME_CMD_INTERNAL_MERGE = 0xff,
	NVME_CMD_INTERNAL_MERGE_WRITE = 0xfe,
} NVME_CMD_OPCODE;
...
typedef enum
{
	...
	FTL_REQUEST_ID_TYPE_HIL_READ,
	FTL_REQUEST_ID_TYPE_WRITE,		// Comon HIL & GC & Meta Write
	FTL_REQUEST_ID_PADDING_READ,
	FTL_REQUEST_ID_PARTIAL_READ,
	FTL_REQUEST_ID_IM_READ,
...
#define BUFFERLPN_COUNT_BITS					8
#define FTL_MOVE_INFO_INDEX_BITS				3
#define REQUEST_BUF_BITS						2
...
typedef struct
{
	...
	unsigned int	bufOffset : REQUEST_BUF_BITS;
} FTL_REQUEST_ID_HIL;
...
{
	unsigned int	nType : FTL_REQUEST_ID_TYPE_BITS;						// FTL_REQUEST_ID_TYPE, common part
	unsigned int	nActiveBlockIndex : ACTIVE_BLOCK_COUNT_BITS;		// Index of Meta Request
	unsigned int	bufferIndex : BUFFERLPN_COUNT_BITS;		// Index of Meta Request
} FTL_REQUEST_ID_PADDING;
...
typedef struct
{
	...
	unsigned int	nActiveBlockIndex : ACTIVE_BLOCK_COUNT_BITS;		// Index of Meta Request
	unsigned int	nRequestIndex : FTL_REQUEST_ID_REQUEST_INDEX_BITS;		// Index of HIL to FTL Request
	unsigned int	bufferIndex : BUFFERLPN_COUNT_BITS;
} FTL_REQUEST_ID_IM;
...
typedef struct
{
	union
	{
		...
		FTL_REQUEST_ID_PADDING		stPadding;
		FTL_REQUEST_ID_IM			stIM;
	};
	INT32						channel;
	INT32						way;
} FTL_REQUEST_ID;
