...
META_CACHE_ENTRY*
META_CACHE::GetMetaEntry(UINT32 nLPN, BOOL bIsGC)
{
	static INT32	nPrevLPN;
	UINT32	nMetaLPN = _GetMetaLPN(nLPN);
    META_CACHE_ENTRY* pstEntry = _HashFind(nMetaLPN);
...
META_CACHE::IsMetaAvailable(UINT32 nLPN, UINT32 FLAG)
{
	...
	if (FLAG == 1)
		pstEntry = GetMetaEntry(mod_lpn, FALSE);
	else
		pstEntry = GetMetaEntry(mod_lpn, TRUE);
...
//{
//    META_CACHE_ENTRY* e;
//    // Iterate all entries and clear confined flag
//    for (INT32 i = 0; i < META_CACHE_ENTRY_COUNT; i++)
//    {
//        if (m_astCacheEntry[i].m_bConfined == TRUE)
//        {
//            m_astCacheEntry[i].m_bConfined = FALSE;
//        }
//    }
//}
...
BOOL
META_CACHE::IsMetaWritable(UINT32 nLPN, UINT32 FLAG)
{
#if (META_PER_WAY==1)
	UINT32 mod_lpn = get_mod_lpn(nLPN);
#else
...
#if (SUPPORT_META_DEMAND_LOADING == 1)
	// [Updated] Check if this is a GC/TRIM request
	BOOL bIsGC = (META_FLAG == 2u);
	bIsGC = (META_FLAG == 1u);

	// [Updated] Pass bIsGC context to GetMetaEntry
...
UINT32
META_CACHE::GetL2V(UINT32 nLPN, UINT32 FLAG)
{
	META_CACHE_ENTRY*		pstEntry;
#if (META_PER_WAY==1)
	UINT32 mod_lpn = get_mod_lpn(nLPN);
...
*/
UINT32
META_CACHE::SetL2V(UINT32 nLPN, UINT32 nVPPN, UINT32 FLAG)
{
	META_CACHE_ENTRY*		pstEntry;
#if (META_PER_WAY==1)
	UINT32 mod_lpn = get_mod_lpn(nLPN);
...
META_CACHE::_Allocate(VOID)
{
	...
	if (m_nFreeCount == 0)
	{
		...
		// [Modified] Find victim from Tail, skipping Confined or IORunning entries
		struct list_head* pHead = &m_dlLRU;
		struct list_head* pPos = pHead->prev; // Start from Tail

		while (pPos != pHead)
		{
			pstEntry = list_entry(pPos, META_CACHE_ENTRY, m_dlList);

			// Skip if IO is running or Entry is Confined (Pinned for GC/TRIM)
			...
			pPos = pPos->prev;
			pstEntry = NULL; // Reset to continue search
		}

		if (pstEntry == NULL)
		{
			// No victim found (All entries are either pinned or doing IO)
			return NULL;
		}
...
BOOL
META_MGR::IsMetaWritable(UINT32 nLPN, UINT32 FLAG)
{
#if (SUPPORT_META_DEMAND_LOADING == 1)
	return m_stMetaCache.IsMetaWritable(nLPN, FLAG);
#else
	return TRUE;
#endif
}
...
UINT32
META_MGR::GetL2V(UINT32 nLPN, UINT32 FLAG)
{
	DEBUG_ASSERT(nLPN < DFTL_GLOBAL::GetInstance()->GetLPNCount());

#if (SUPPORT_META_DEMAND_LOADING == 1)
	return m_stMetaCache.GetL2V(nLPN, FLAG);
#else
...
#if (SUPPORT_META_DEMAND_LOADING == 1)
	nOldVPPN = m_stMetaCache.SetL2V(nLPN, nVPPN, DS_FLAG);
#else
