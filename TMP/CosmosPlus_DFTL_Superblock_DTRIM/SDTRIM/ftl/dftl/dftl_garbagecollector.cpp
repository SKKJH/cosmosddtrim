...
//		xil_printf("	[1] Result Reduce Page: %u, Load CMT: %u\r\n", beforeVPC - 0, FLAG);
...
//			xil_printf("	[2] Result Reduce Page: %u, Load CMT: %u\r\n", beforeVPC - afterVPC, FLAG);
...
//	if (m_nIssuedCount == m_nWriteCount + 1) {
		return;
	}
	do
	{
...
//					}
		...
		else if (m_nCurReadVPageOffset > pstGlobal->GetVPagePerVBlock())
		{
			if (m_eIOType != IOTYPE_META)
			{
				if (m_nWriteCount == m_nIssuedCount)
