...
SUPER_GC_MGR::CheckAndStartGC()
{
	...
	UINT32 beforeVBN = GetVictimVBN();
	m_nTMPVictimVBN = beforeVBN;
	UINT32 bef_cid = pstGlobal->GetSBInfoMgr()->m_pastSBInfo[beforeVBN].m_bCID;
	UINT32 beforeVPC = victimVPC;

	UINT32 min_cid = 0u;
	UINT32 min_pct = 101u;

	if ((TrimPending_IsOn()) && (!g_trim_going))
	{
		...
		gc_trim = 1;
		g_trim_going = 1;
//		FLAG = FindTopKTrimSegments(16);
//		FLAG = FindTopKTrimSegments(16, min_cid);
		FLAG = FindTopKTrimSegments_Pri(16, min_cid);
		CheckPendingTrim();
	...
	if (nFreeBlock > m_nThreshold)
	{
		...
		DFTL_IncreaseProfile(Prof_DS_UTIL_Hit);
		DFTL_IncreaseProfile(Prof_DS_Reduced_Pages, beforeVPC);
		return 0;
	}

	UINT32 CandVBN = GetVictimVBN();
	UINT32 afterVPC = victimVPC;
	...
	if (CandVBN != INVALID_VBN)
	{
		...
		if (FLAG != INVALID_VBN)
		{
			if (beforeVPC == afterVPC)
				DFTL_IncreaseProfile(Prof_DS_UTIL_Miss);
			else
				DFTL_IncreaseProfile(Prof_DS_UTIL_Hit);
			if (FLAG == vic_cid)
				DFTL_IncreaseProfile(Prof_DS_Cluster_Hit);
			else
				DFTL_IncreaseProfile(Prof_DS_Cluster_Miss);

			if (FLAG == bef_cid)
				DFTL_IncreaseProfile(Prof_DS_Victim_Hit);
			else
				DFTL_IncreaseProfile(Prof_DS_Victim_Miss);
			DFTL_IncreaseProfile(Prof_DS_Reduced_Pages, beforeVPC - afterVPC);
		}
		return 1;
	}
	else
...
GC_MGR::IncreaseWriteCount(VOID)
{
	...
	if (m_nWriteCount == m_nVPC)
	{
		...
		else
		{
			...
			{
//			    xil_printf("[GC END'1 Before Release] VBN:%u(%d USED), FREE:%u\r\n",
//			               pastVBN, DFTL_GLOBAL::GetSBInfoMgr()->m_pastSBInfo[pastVBN].m_nUSED, DFTL_GLOBAL::GetSBInfoMgr()->m_nFreeCount);
				DFTL_GLOBAL::GetInstance()->GetUserBlockMgr()->Release(m_channel, m_way, pastVBN, 1);
//			    xil_printf("[GC END'1 After Release] VBN:%u(%d USED), FREE:%u\r\n",
//			               pastVBN, DFTL_GLOBAL::GetSBInfoMgr()->m_pastSBInfo[pastVBN].m_nUSED, DFTL_GLOBAL::GetSBInfoMgr()->m_nFreeCount);
			}
			...
			{
				auto get_vcnt = [&](int ch, int wy) -> UINT32 {
					UINT32 v = -1;
					auto *mgr = DFTL_GLOBAL::GetVBInfoMgr(ch, wy);
					if (mgr) {
						VBINFO *vb = mgr->GetVBInfo(pastVBN);
						if (vb) {
							bool is_free = false;
							is_free = vb->IsFree();
							if (!is_free) {
								v = 512 - vb->GetInvalidLPNCount();
							}
						}
					}
					return v;
...
//			    xil_printf("[GC END'1] VBN:%u(%d USED), VCNT:[%u, %u, %u, %u], FREE:%u\r\n",
//			               pastVBN,
...
//						   DFTL_GLOBAL::GetSBInfoMgr()->m_nFreeCount);
	    		...
	    		if (DFTL_GLOBAL::GetSuperGCMgr()->gc_trim == 1)
	    		{
	    			DFTL_GLOBAL::GetSuperGCMgr()->gc_trim = 0;
	    			g_trim_going = 0;
	    		}
			}
		}
		...
		if (m_nCurReadVPageOffset >= pstGlobal->GetVPagePerVBlock())
		{
			if (m_eIOType != IOTYPE_META)
			{
				if (m_nWriteCount == m_nIssuedCount)
				{
					...
					{
//					    xil_printf("[GC END'2 Before Release] VBN:%u(%d USED), FREE:%u\r\n",
//					    		m_nVictimVBN, DFTL_GLOBAL::GetSBInfoMgr()->m_pastSBInfo[m_nVictimVBN].m_nUSED, DFTL_GLOBAL::GetSBInfoMgr()->m_nFreeCount);
						DFTL_GLOBAL::GetInstance()->GetUserBlockMgr()->Release(m_channel, m_way, m_nVictimVBN, 1);
//					    xil_printf("[GC END'2 After Release] VBN:%u(%d USED), FREE:%u\r\n",
//					    		m_nVictimVBN, DFTL_GLOBAL::GetSBInfoMgr()->m_pastSBInfo[m_nVictimVBN].m_nUSED, DFTL_GLOBAL::GetSBInfoMgr()->m_nFreeCount);
					}
...
//						auto get_vcnt = [&](int ch, int wy) -> UINT32 {
//							UINT32 v = -1;
//							auto *mgr = pstGlobal->GetVBInfoMgr(ch, wy);
//							if (mgr) {
//								VBINFO *vb = mgr->GetVBInfo(m_nVictimVBN);
//								if (vb) {
//									bool is_free = false;
//									is_free = vb->IsFree();
//									if (!is_free) {
...
//					    xil_printf("[GC END'2] VBN:%u(%d USED), VCNT:[%u, %u, %u, %u], FREE:%u\r\n",
//					    		m_nVictimVBN,
