...
// Hash kjh
	...
	for (INT32 i = 0; i < META_CACHE_ENTRY_COUNT; i++) //META_CACHE_ENTRY_COUNT: 80
	{
...
META_CACHE::Format(VOID)
{
	...
	do
	{
		...
		META_CACHE_ENTRY*	pstEntry = _Allocate(0);
		if (pstEntry == NULL)
		{
			bRet = FALSE;
			break;
...
//	if (FLAG == 1)
//	{
		if (DFTL_GLOBAL::GetInstance()->trace == 1)
		{
			UINT32 Page_Hit = DFTL_GetProfile(Prof_Page_Hit);
			UINT32 Page_Miss = DFTL_GetProfile(Prof_Page_Miss);
			UINT32 GC_Hit = DFTL_GetProfile(Prof_GC_Hit);
			UINT32 GC_Miss = DFTL_GetProfile(Prof_GC_Miss);

			if ((Page_Hit + Page_Miss + GC_Hit + GC_Miss) < 1000)
			{
				if (FLAG == 1)
				{
					if (pstEntry->m_bValid == TRUE)
						DFTL_IncreaseProfile(Prof_Page_Hit);
					else
						DFTL_IncreaseProfile(Prof_Page_Miss);
				}
				else
				{
					if (pstEntry->m_bValid == TRUE)
						DFTL_IncreaseProfile(Prof_GC_Hit);
					else
						DFTL_IncreaseProfile(Prof_GC_Miss);
				}
			}
...
//			xil_printf("[PROF] CMT HIT RATE [%u/%u]\r\n", Page_Hit, Page_Hit + Page_Miss);
//			DFTL_GLOBAL::GetInstance()->trace += 1;

//			if (DFTL_GLOBAL::GetInstance()->trace >= 100)
//			{
//		    	DFTL_ResetProfile(Prof_Page_Miss);
//		    	DFTL_ResetProfile(Prof_Page_Hit);
//			}
		}
//	}
...
//		xil_printf("Im Here\r\n");
	...
	pstRequest->Initialize(META_REQUEST_READ_WAIT, pstEntry->m_nMetaLPN, pstEntry, IOTYPE_META, DS_FLAG);
#if (META_PER_WAY==1)
	UINT32 channel, way;
...
*/
UINT32
META_CACHE::SetL2V(UINT32 nLPN, UINT32 nVPPN, UINT32 D_FLAG)
{
	META_CACHE_ENTRY*		pstEntry;
#if (META_PER_WAY==1)
	UINT32 mod_lpn = get_mod_lpn(nLPN);
...
META_CACHE_ENTRY*
META_CACHE::_Allocate(UINT32 D_FLAG)
{
	META_CACHE_ENTRY*	pstEntry;
	if (m_nFreeCount == 0)
	{
		...
		if (pstEntry->m_bDirty == TRUE)
		{
			...
			pstRequest->Initialize(META_REQUEST_WRITE_WAIT, pstEntry->m_nMetaLPN, pstEntry, IOTYPE_META, 0);
...
META_MGR::Initialize(VOID)
{
	...
	m_bFormatted = FALSE;
	xil_printf("META CACHE NUM: %u\r\n", m_stMetaCache.Capacity());
}
...
META_MGR::ApplyTrimOnLoadedEntry(META_CACHE_ENTRY* pEntry, UINT32 metaLPN)
{
    ...
    for (INT32 i = 0; i < (INT32)L2V_PER_META_PAGE; ++i)
    {
		...
		pstVNand->Invalidate(oldVPPN);
		DFTL_GLOBAL::GetInstance()->GetUserBlockMgr()->Invalidate(oldVPPN, 2);
...
META_MGR::ApplyTrimOnLoadedEntry_OPT(META_CACHE_ENTRY* pEntry, UINT32 metaLPN)
{
    ...
    for (UINT32 b = byte_start; b < byte_end_ex; ++b) {
        ...
        for (UINT32 bit = bit_lo; bit <= bit_hi; ++bit) {
            ...
            if (DFTL_GLOBAL::GetGCMgr(channel, way)->GetVBN() == nVBN) continue;

            // L2V를 INVALID로 (정렬 이슈 대비)
            _safe_store_u32(&pEntry->m_anL2V[i], INVALID_PPN);
            touched = true;

            // 나중에 Invalidate할 목록에 추가
            if (inv_cnt < (UINT32)L2V_PER_META_PAGE) {
                old_vppns[inv_cnt] = oldVPPN;
//                old_lpns [inv_cnt] = start_lpn + i;
                ++inv_cnt;
                TRIM_CNT += 1;
            }
    ...
    // ---- 여기서 바로 Invalidate 수행 ----
    if (inv_cnt) {

        for (UINT32 k = 0; k < inv_cnt; ++k) {
        #if defined(DEBUG) || defined(_DEBUG)
            // nLPN 오타 방지: 반드시 lpn과 비교
            DEBUG_ASSERT(v->GetV2L(old_vppns[k]) == old_lpns[k]);
        #endif
            v->Invalidate(old_vppns[k]);
            ubm->Invalidate(old_vppns[k], 2);
        }
    }

    // 비트맵 정리(세그 카운트까지 반영하는 기존 함수)
    ClearTrimRange(start_lpn, end_lpn);
//    xil_printf("	INVALID %u Pages\r\n", TRIM_CNT);
}
...
VOID
META_MGR::SetL2V(UINT32 nLPN, UINT32 nVPPN, UINT32 D_FLAG)
{
	DEBUG_ASSERT(nLPN < DFTL_GLOBAL::GetInstance()->GetLPNCount());
...
#if (SUPPORT_META_DEMAND_LOADING == 1)
	nOldVPPN = m_stMetaCache.SetL2V(nLPN, nVPPN, D_FLAG);
	...
	else
	{
		...
		DFTL_GLOBAL::GetInstance()->GetUserBlockMgr()->Invalidate(nOldVPPN, D_FLAG);
	}
}
...
META_L2V_MGR::SetL2V(UINT32 nLPN, UINT32 nVPPN)
{
	...
	if (m_panL2V[nLPN] != INVALID_PPN)
	{
		...
		DFTL_GLOBAL::GetInstance()->GetMetaBlockMgr()->Invalidate(nOldVPPN, 0);
