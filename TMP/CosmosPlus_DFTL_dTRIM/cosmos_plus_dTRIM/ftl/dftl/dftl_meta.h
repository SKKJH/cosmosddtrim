...
public:
	UINT32 Capacity(VOID) const {
	    return (UINT32)(sizeof(m_astCacheEntry) / sizeof(m_astCacheEntry[0]));
	}

	VOID				Initialize(VOID);
	BOOL				Format(VOID);
	BOOL				IsMetaAvailable(UINT32 nLPN, UINT32 FLAG);
	BOOL				IsMetaWritable(UINT32 nLPN);
	META_CACHE_ENTRY*	GetMetaEntry(UINT32 nLPN);
	BOOL				LoadMeta(UINT32 nLPN, UINT32 DS_FLAG);

	UINT32	GetL2V(UINT32 nLPN);
	UINT32	SetL2V(UINT32 nLPN, UINT32 nVPPN, UINT32 D_FLAG);
	UINT32  get_mod_lpn(UINT32 nLPN);

private:
	META_CACHE_ENTRY*	_Allocate(UINT32 D_FLAG);
...
public:
	...
	BOOL	Format(VOID);
	UINT32	GetL2V(UINT32 nLPN);
	VOID	SetL2V(UINT32 nLPN, UINT32 nVPPN, UINT32 D_FLAG);
