...
#include "xtime_l.h"   // 파일 상단 어딘가에 한 번만 포함
...
//	u32   ns_check;
//
//	const u64 TICKS_PER_US = 500ULL;   // 5e8 / 1e6 = 500
//	const u64 NS_PER_TICK  = 2ULL;     // 1 tick = 2 ns
	...
	if ((TrimPending_IsOn()) && (!g_trim_going) && (m_eIOType == IOTYPE_GC))
	{
//		if (DFTL_GLOBAL::GetInstance()->print_cnt <= 10)
//		{
//			do {
//				REQUEST_MGR* pstReqMgr = DFTL_GLOBAL::GetInstance()->GetRequestMgr();
//
//				pstReqMgr->_ProcessHILRequestWaitQ();
...
//				}
//
//				HIL_REQUEST* pstHILReq = pstReqMgr->GetHILRequestInfo()->GetDoneRequest();
//				if (pstHILReq != NULL) {
//					pstHILReq->Run();
//				}
//
//				GC_REQUEST* pstGCReq = pstReqMgr->GetGCRequestInfo()->GetDoneRequest();
//				if (pstGCReq != NULL) {
//					pstGCReq->Run();
//				}
//
//				META_REQUEST* pstMETAReq = pstReqMgr->GetMetaRequestInfo()->GetDoneRequest();
//				if (pstMETAReq != NULL) {
//					pstMETAReq->Run();
//				}
//
//				if (pstHILReq == NULL && pstGCReq == NULL && pstMETAReq == NULL) {
//					break;
//				}
//			} while (1);
//		}
		...
		if (DFTL_GLOBAL::GetInstance()->trace == 1)
		{
			...
			xil_printf("[PROF] CMT HIT RATE [%u/%u]\r\n", Page_Hit + GC_Hit, Page_Hit + Page_Miss + GC_Hit + GC_Miss);
			xil_printf("[PROF] HOST HIT [%u], GC HIT [%u]\r\n", Page_Hit, GC_Hit);
			xil_printf("[PROF] HOST MISS [%u], GC MISS [%u]\r\n", Page_Miss, GC_Miss);
			DFTL_ResetProfile(Prof_Page_Miss);
			DFTL_ResetProfile(Prof_Page_Hit);
			DFTL_ResetProfile(Prof_GC_Miss);
			DFTL_ResetProfile(Prof_GC_Hit);
			DFTL_GLOBAL::GetInstance()->trace = 0;
		}

		gc_trim = 1;
		g_trim_going = 1;

		UINT32 SegmentCnt = FindTopKTrimSegments(64);
		CheckPendingTrim();
...
//    {
//    	if (m_eIOType != IOTYPE_META)
//    		xil_printf("	DATA GC START// m_nVictimVBN: %u/ m_nVPC: %u\r\n", m_nVictimVBN, m_nVPC);
//    }
}
