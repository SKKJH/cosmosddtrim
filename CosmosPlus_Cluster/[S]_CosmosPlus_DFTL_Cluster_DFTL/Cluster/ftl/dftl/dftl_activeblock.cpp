...
VOID
PROGRAM_UNIT::Add(UINT32 cID, VOID* pstRequest, UINT32 nVPPN, IOTYPE eBlockType, IOTYPE eRequestType)
{
	if (m_nLPNCount == 0)
	{
...
VOID
ACTIVE_BLOCK::_IssueProgram(UINT32 cID, IOTYPE GC_TYPE)
{
	...
	stProgramReqID.cluster = cID;
	stProgramReqID.stProgram.nType = FTL_REQUEST_ID_TYPE_WRITE;
	stProgramReqID.stProgram.nActiveBlockIndex = DFTL_GLOBAL::GetActiveBlockMgr(cID, stProgramReqID.channel, stProgramReqID.way)->GetActiveBlockIndex(m_eBlockType);
	stProgramReqID.stProgram.nBufferingIndex = m_nCurProgramBuffering;
	stProgramReqID.stProgram.nIOType = m_eBlockType;

	// do write
	DFTL_GLOBAL::GetVNandMgr()->ProgramPage(stProgramReqID, pstProgramUnit, m_eBlockType, GC_TYPE);
...
*/
BOOL
ACTIVE_BLOCK::Write(VOID* pstRequest, UINT32 cID, IOTYPE eRequestType, IOTYPE GC_TYPE)
{
	...
	// Add to buffering
	pstProgramUnit->Add(cID, pstRequest, m_nCurVPPN, m_eBlockType, eRequestType);

	if (m_eBlockType == IOTYPE_HOST)
	{
		HIL_REQUEST_PER_WAY*	pstHILRequest = static_cast<HIL_REQUEST_PER_WAY*>(pstRequest);
		// Update Mapping
		DFTL_GLOBAL::GetMetaMgr()->SetL2V(pstHILRequest->GetCurLPN(), m_nCurVPPN, 0, FALSE);

		// host request only
		BUFFERING_LPN* pstBufferingLPN = DFTL_GLOBAL::GetActiveBlockBufferingLPN(cID, pstHILRequest->GetChannel(), pstHILRequest->GetWay());
	...
	else if (m_eBlockType == IOTYPE_GC)
	{
		...
		// Update Mapping
		DFTL_GLOBAL::GetMetaMgr()->SetL2V(pstGCRequest->GetLPN(), m_nCurVPPN, 0, GC_OPT);

		BUFFERING_LPN* pstBufferingLPN = DFTL_GLOBAL::GetActiveBlockBufferingLPN(cID, pstGCRequest->GetChannel(), pstGCRequest->GetWay());
		pstBufferingLPN->Add(pstGCRequest->GetLPN());
	}
#if (SUPPORT_META_DEMAND_LOADING == 1)
	else if (m_eBlockType == IOTYPE_META)
	{
		...
		channel = CHANNEL_FROM_VPPN(m_nCurVPPN);
		way = WAY_FROM_VPPN(m_nCurVPPN);
	...
	{
		// Issue
		_IssueProgram(cID, GC_TYPE);
	}

	UINT32 b_CurVPPN = m_nCurVPPN;
	m_nCurVPPN++;

	// check boundary of VBlock
	if (VPN_FROM_VPPN(m_nCurVPPN) == 0)
	{
		UINT32 CH = CHANNEL_FROM_VPPN(b_CurVPPN);
		UINT32 WAY = WAY_FROM_VPPN(b_CurVPPN);
		UINT32 VBN = VBN_FROM_VPPN(b_CurVPPN);
		DFTL_GLOBAL::GetInstance()->GetUserBlockMgr()->INIT_CVPC(CH, WAY, VBN);
		// end of vblock
...
ACTIVE_BLOCK::ProgramDone(INT32 nBufferingIndex)
{
	...
	cID = pstProgramUnit->GetCID();

	BUFFERING_LPN*	pstBufferingLPN;

	if (m_eBlockType == IOTYPE_HOST || m_eBlockType == IOTYPE_GC)
	{
		pstBufferingLPN = DFTL_GLOBAL::GetActiveBlockBufferingLPN(cID, channel, way);
	}
#if (SUPPORT_META_DEMAND_LOADING == 1)
	else
	{
		pstBufferingLPN = DFTL_GLOBAL::GetMetaActiveBlockBufferingLPN(0, channel, way);
	}
#else
	else
	{
...
VOID
ACTIVE_BLOCK::InvalidateLPN(UINT32 nLPN)
{
    for (INT32 i = 0; i < ACTIVE_BLOCK_BUFFERING_COUNT; i++)
    {
        PROGRAM_UNIT* pstProgramUnit = &m_astBuffering[i];
        if (pstProgramUnit->m_bFree == TRUE || pstProgramUnit->m_bProgramming == TRUE)
        {
            continue;
        }

        // 유닛 내부의 LPN들을 확인
        for (UINT32 j = 0; j < pstProgramUnit->m_nLPNCount; j++)
        {
            if (pstProgramUnit->GetLPN(j) == nLPN)
            {
                UINT32 cID = pstProgramUnit->GetCID();
                UINT32 channel = CHANNEL_FROM_VPPN(pstProgramUnit->GetVPPN());
                UINT32 way = WAY_FROM_VPPN(pstProgramUnit->GetVPPN());

                BUFFERING_LPN* pstBufferingLPN = DFTL_GLOBAL::GetActiveBlockBufferingLPN(cID, channel, way);
                pstBufferingLPN->Remove(nLPN);

                pstProgramUnit->InvalidateLPN(j);
                return;
            }
        }
    }
...
#if defined(SUPPORT_DATA_VERIFICATION) || defined(WIN32)
				if (m_eBlockType != IOTYPE_META)
				{
					((UINT32*)pDataAddr)[0] = ((UINT32*)pstBufferEntry->m_pSpareBuf)[nLPNIndex];
				}
...
*/
UINT32
BUFFERING_LPN::ReadLPN(UINT32 nLPN, BUFFER_ENTRY* pstBufferEntry, UINT32 cID, UINT32 channel, UINT32 way)
{
	UINT16	nIndex = BUFFERING_LPN::GET_BUCKET_INDEX(nLPN);
//	UINT32 channel, way;
...
///////////////////////////////////////////////////////////////////////////////
VOID
ACTIVE_BLOCK_MGR::Initialize(UINT32 cluster, UINT32 channel, UINT32 way)
{
	m_nClusterID			= cluster;
...
ACTIVE_BLOCK*
ACTIVE_BLOCK_MGR::_GoToNextActiveBlock(IOTYPE eIOType, UINT32 nextVBN)
{
	ACTIVE_BLOCK*		pstActiveBlock;
	BOOL				bUser = FALSE;
	BOOL				bGC = FALSE;
	...
	{
		VBINFO* pstVBInfo = DFTL_GLOBAL::GetVBInfoMgr(m_nchannel, m_nway)->GetVBInfo(pstActiveBlock->m_nVBN);
		DEBUG_ASSERT(pstVBInfo->IsActive() == TRUE);

		pstVBInfo->ClearActive();
#if (SUPPORT_BLOCK_DEBUG == 1)
		pstBlockMgr->CheckVPC(m_nchannel, m_nway, pstActiveBlock->m_nVBN);
#endif
	}

	// Allocate New Block
	UINT32 nVBN = pstBlockMgr->Allocate(m_nchannel, m_nway, bUser, bGC, bMeta, nextVBN, 1, m_nClusterID);
	pstActiveBlock->m_nVBN = nVBN;
	pstActiveBlock->m_nCurVPPN = GET_VPPN_FROM_VPN_VBN(m_nchannel, m_nway, 0, nVBN);
	...
	{
	case IOTYPE_HOST:
		DFTL_GLOBAL::GetVBInfoMgr(m_nchannel, m_nway)->GetVBInfo(nVBN)->CID = m_nClusterID;
		DFTL_GLOBAL::GetInstance()->IncreaseProfileCount(PROFILE_IO_ERASE, 1);
		break;

	case IOTYPE_GC:
		DFTL_GLOBAL::GetVBInfoMgr(m_nchannel, m_nway)->GetVBInfo(nVBN)->CID = m_nClusterID;
		DFTL_GLOBAL::GetInstance()->IncreaseProfileCount(PROFILE_GC_ERASE, 1);
		break;

	case IOTYPE_META:
		DFTL_GLOBAL::GetVBInfoMgr(m_nchannel, m_nway)->GetVBInfo(nVBN)->CID = 0;
		DFTL_GLOBAL::GetInstance()->IncreaseProfileCount(PROFILE_META_ERASE, 1);
		break;
	...
	if (pstActiveBlock->m_nCurVPPN == INVALID_PPN)
	{
		if (eIOType != IOTYPE_META)
		{
			...
			for (int i=0; i<USER_CHANNELS; i++)
			{
				...
				{
					UINT32 wp = DFTL_GLOBAL::GetInstance()->GetActiveBlockMgr(m_nClusterID, i,j)->_GetCurActiveBlock(eIOType)->m_nCurVPPN;
					if (wp != INVALID_VPPN)
						check = 1;
				}
			}
			...
			if (check == 1) {
				static unsigned ab_null_ctr = 0;
				ab_null_ctr++;

				// 1,2,4,8,16,...번째 NULL에서만 출력 (폭주 방지 최고)
				if ((ab_null_ctr & (ab_null_ctr - 1)) == 0) {
				    xil_printf("[GC][AB] GetActiveBlock NULL (pow2) cid=%u ch=%u wy=%u io=%u\n",
				           (unsigned)m_nClusterID, (unsigned)m_nchannel, (unsigned)m_nway, (unsigned)eIOType);
				}
				return NULL;
			}
			...
			for (int i = 0; i < USER_CHANNELS; i++) {
				for (int j = 0; j < USER_WAYS; j++) {
					ACTIVE_BLOCK_MGR* mgr = DFTL_GLOBAL::GetInstance()->GetActiveBlockMgr(m_nClusterID, i, j);
...
ACTIVE_BLOCK_MGR::ReadBufferingLPN(UINT32 nLPN, BUFFER_ENTRY* pstBufferEntry, IOTYPE eIOType)
{
	ACTIVE_BLOCK*	pstActiveBlock;
	UINT32			nVPPN;
	UINT32			targetVPPN = INVALID_VPPN;
	if (eIOType != IOTYPE_META) {
		targetVPPN = DFTL_GLOBAL::GetMetaMgr()->GetL2V(nLPN, FALSE);
		nVPPN = m_pstCurActiveBlockHost->ReadBufferingLPN(nLPN, pstBufferEntry);
		if (nVPPN != targetVPPN) {
			nVPPN = m_pstCurActiveBlockGC->ReadBufferingLPN(nLPN, pstBufferEntry);
		}
		DEBUG_ASSERT((nVPPN == INVALID_VPPN) || nVPPN == targetVPPN);
	}
	else {
		nVPPN = m_pstCurActiveBlockMeta->ReadBufferingLPN(nLPN, pstBufferEntry);
	}
	return nVPPN;
}
