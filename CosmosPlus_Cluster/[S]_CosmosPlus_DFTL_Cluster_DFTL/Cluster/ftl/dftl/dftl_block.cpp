		...
		for (UINT32 way = 0; way < USER_WAYS; way++)
		{
			...
			for (UINT32 i = 0; i < nVBlockCount; i++)
			{
				...
				else
				{
					...
					pstVBInfo->ClearFree();
					pstVBInfo->ClearGC();
					DFTL_GLOBAL::GetSBInfoMgr()->m_pastSBInfo[i].m_bFree = FALSE;
...
UINT32
BLOCK_MGR::Allocate(UINT32 channel, UINT32 way, BOOL bUser, BOOL bGC, BOOL bMeta, UINT32 nextVBN, UINT32 FLAG, UINT32 CID)
{
	if ((FLAG != 1) || (bMeta == 1))
	{
...
VOID BLOCK_MGR::Release(UINT32 channel, UINT32 way, UINT32 nVBN, UINT32 FLAG)
{
	...
	if ((FLAG != 0) && !pstVBInfo->IsMeta())
	{
	    ...
	    if (sb->m_nUSED <= 0) {
	        xil_printf("\t[WARN %u] Release underflow: VBN:%u USED CNT:%d\r\n", FLAG, nVBN, sb->m_nUSED);
	    } else {
	        sb->m_nUSED -= 1;
	    }
