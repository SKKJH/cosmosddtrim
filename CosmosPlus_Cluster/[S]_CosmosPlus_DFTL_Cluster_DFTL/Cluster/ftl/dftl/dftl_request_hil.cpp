...
HIL_REQUEST::_ProcessDSM_Wait(VOID)
{
    ...
    META_MGR* pstMetaMgr = DFTL_GLOBAL::GetMetaMgr();
    BOOL bTrimEnabled = (DFTL_GLOBAL::GetInstance()->m_bEnable > 1);
    ...
    if (bTrimEnabled)
    {
    	...
    	for (UINT32 i = 0; i < m_nLPNCount; i++)
    	{
    	    ...
    	    for (UINT32 j = 0; j < nLength; ++j) {
    	    	currentCmdPageCount++;
                ck = false;
    	    	if (pstMetaMgr->IsMetaAvailable(nCurLPN, TRIM_OPT) == FALSE) {
...
//                }
    	    	...
    	    	while (pstMetaMgr->IsMetaAvailable(nCurLPN, TRIM_OPT) == FALSE)
    	    	{
    	    		pstMetaMgr->LoadMeta(nCurLPN, 1, TRIM_OPT);
    	    	...
    	    	UINT32 nCurVPPN = pstMetaMgr->GetL2V(nCurLPN, TRIM_OPT);
    	    	if (nCurVPPN != INVALID_PPN) {
                    ...
                    XTime_GetTime(&tStart);
    	    		pstMetaMgr->SetL2V(nCurLPN, INVALID_PPN, 1, TRIM_OPT);
                    XTime_GetTime(&tEnd);
    	    ...
    	    if(first_ck) {
    	        DFTL_IncreaseProfile(Prof_Discard_Miss_Range_Num);
    	    } else {
    	        DFTL_IncreaseProfile(Prof_Discard_Hit_Range_Num);
    	    }
    	}
//    	DFTL_GLOBAL::GetInstance()->StartGCMonitor(1000, 100);
    ...
    if (bTrimEnabled)
    {
    	DFTL_GLOBAL::GetInstance()->UpdateTrimStat(
    	        currentCmdPageCount, currentCmdLoadCount, currentCmdWBHitCount, t_dma, t_cmt, t_wb, t_l2v
    	);
    }
    return TRUE;
}
