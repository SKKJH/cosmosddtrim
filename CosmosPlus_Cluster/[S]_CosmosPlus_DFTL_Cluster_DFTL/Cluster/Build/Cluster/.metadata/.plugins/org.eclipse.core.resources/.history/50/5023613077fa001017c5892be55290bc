...
#define MAX_NUM_OF_IO_SQ	1
#define MAX_NUM_OF_IO_CQ	1
...
#define HOST_BLOCK_SIZE					4096		// 4K BLOCK
...
{
	unsigned short MS;
	unsigned char LBADS;							// dy, size of LBA, power of 2
	unsigned char RP				:2;
...
typedef struct _IO_READ_COMMAND_DW12
{
	union {
		unsigned int dword;
		struct {
			unsigned short NLB;									// number of block, 0: 1block
			unsigned short reserved0				:10;
			unsigned short PRINFO					:4;			// dy, protection information
			unsigned short FUA						:1;			// dy, Force Unit Access
			unsigned short LR						:1;			// dy, limited retry
		};
	};
