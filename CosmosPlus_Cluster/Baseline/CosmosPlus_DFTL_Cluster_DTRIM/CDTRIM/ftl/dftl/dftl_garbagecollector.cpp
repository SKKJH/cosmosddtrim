...
SUPER_GC_MGR::CheckAndStartGC()
{
    ...
    BOOL bTrimTrigger;
    if (BASE_LINE)
    	bTrimTrigger = (pstGlobal->m_nTotalPendingTrimCount > 0);
    else
    	bTrimTrigger = 0;
...
GC_MGR::_Read(VOID)
{
	...
	do
	{
		...
		if (bValid == TRUE)
		{
			...
			if (pstGlobal->IsValidLPN(nLPN) && g_trim_bitmap != NULL)
			{
				UINT32 nByteIdx = nLPN / 8;
				UINT32 nBitIdx  = nLPN % 8;
...
//					xil_printf("	TRIM PASS LPN: %u\r\n", nLPN);
					...
					g_trim_bitmap[nByteIdx] &= ~(1 << nBitIdx);
