...
#else
	HIL_ReadBlock(cmdSlotTag, startLba[0], (nlb + 1));
#endif
}
...
#else
	HIL_WriteBlock(cmdSlotTag, startLba[0], (nlb + 1));
#endif
}
...
void handle_nvme_io_dsm(unsigned int cmdSlotTag, NVME_IO_COMMAND *nvmeIOCmd)
{
	...
	HIL_DatasetManagement(cmdSlotTag, nr, ad);
}

// dy, (read)IO command를 slice(Mapping Unit) 단위로 나누어 request를 생성하고 sliceReqQ에 추가.
