...
struct CMT_GROUP_NODE
{
    ...
    UINT32 m_nTotalTrim;    // 누적 TRIM 양 (Value)
    CMT_GROUP_NODE* m_pNext;
    CMT_GROUP_NODE* m_pPrev;
...
struct TRIM_NODE
{
    ...
    // 초기화 헬퍼
    void Reset() {
        m_nStartLPN = 0xFFFFFFFF; // INVALID
        m_nLength = 0;
        m_pPrevLPN = NULL;
        m_pNextLPN = NULL;
        m_pPrevRange = NULL;
...
// ----------------------------------------------------------------------------
// 2. TRIM MANAGER 클래스 정의 (Cluster 별로 하나씩 존재)
// ----------------------------------------------------------------------------
class TRIM_MGR
{
public:
    TRIM_MGR();
    void Initialize(UINT32 clusterID);
    void PrintInfo();
    VOID InvalidateOverlap(UINT32 nWriteStart, UINT32 nWriteLength);
    VOID VisualPrintBuckets();
    ...
    void _UpdateNodeLengthOnly(TRIM_NODE* pNode, UINT32 nNewLength);
...
{
public:
	BOOL   m_bFormatCheck;        // 모니터링 활성화 여부
	...
	TRIM_NODE* m_pstTrimNodePool;        // 10,000개 노드 실제 메모리
	TRIM_NODE* m_pGlobalFreeListHead;   // 전역 프리 리스트 헤드
	UINT32     m_nGlobalUsedNodeCount;  // 시스템 전체 사용 노드 수

	// [신규] 통합 할당/해제 함수 (교체 정책 포함)
	TRIM_NODE* AllocTrimNode(UINT32 nLength, UINT32 nRequestClusterID);
	void FreeTrimNode(TRIM_NODE* pNode);

	// [기존 유지]
	VOID VisualPrintAllTrimNodes();

	TRIM_MGR m_stTrimMgr[USER_CLUSTERS];
	TRIM_MGR* GetTrimMgr(UINT32 clusterID) {
		if(clusterID >= USER_CLUSTERS) return NULL;
		return &m_pstInstance->m_stTrimMgr[clusterID];
	}
