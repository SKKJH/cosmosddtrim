...
HIL_REQUEST::_ProcessWrite_Wait(VOID)
{
   ...
   if (TrimPending_IsOn())
   {
	   ...
	   UINT32 nTotalLPNs = pGlobal->GetLPNCount();
	   UINT32 nLPNsPerCluster = nTotalLPNs / USER_CLUSTERS;

	   UINT32 nCurLPN = m_nLPN;
	   UINT32 nRemainingCount = m_nLPNCount;

	   while (nRemainingCount > 0)
	   {
		   UINT32 nClusterID = pGlobal->GetClusterID(nCurLPN);
		   UINT32 nClusterEndLPN = (nClusterID + 1) * nLPNsPerCluster;

		   UINT32 nProcessLen = nRemainingCount;
		   if (nCurLPN + nRemainingCount > nClusterEndLPN)
		   {
			   nProcessLen = nClusterEndLPN - nCurLPN;
		   }

		   TRIM_MGR* pstTrimMgr = pGlobal->GetTrimMgr(nClusterID);
		   if (pstTrimMgr != NULL && pstTrimMgr->m_nUsedNodeCount > 0)
		   {
			   pstTrimMgr->InvalidateOverlap(nCurLPN, nProcessLen);
		   }

		   nCurLPN += nProcessLen;
		   nRemainingCount -= nProcessLen;
	   }
   }
...
HIL_REQUEST::_ProcessDSM_Wait(VOID)
{
    ...
    if (DFTL_GLOBAL::GetInstance()->m_bEnable > 1)
    {
    	for (UINT32 i = 0; i < m_nLPNCount; i++)
    	{
    		...
    		{
    			if (pstMetaMgr->IsMetaAvailable(nCurLPN, TRIM_OPT))
    			{
