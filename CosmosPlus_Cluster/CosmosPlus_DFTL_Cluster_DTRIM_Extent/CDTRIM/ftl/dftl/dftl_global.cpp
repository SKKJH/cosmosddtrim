...
// ----------------------------------------------------------------------------
// [DFTL_GLOBAL] 통합 TRIM 노드 할당/해제 및 교체 정책
// ----------------------------------------------------------------------------
...
TRIM_NODE* DFTL_GLOBAL::AllocTrimNode(UINT32 nLength, UINT32 nRequestClusterID)
{
	...
	if (pVictim != NULL) {
        // [CRITICAL FIX] 반드시 기존 리스트에서 먼저 연결을 끊어야 함!!!
        m_stTrimMgr[nVictimClusterID]._RemoveFromLists(pVictim);

        // Victim 노드 해제 및 정보 갱신 (메타 정보 갱신 포함)
        m_stTrimMgr[nVictimClusterID].FreeNode(pVictim);

        // 다시 할당 시도 (FreeNode 호출로 Free List에 반환되었으므로)
        if (m_pGlobalFreeListHead != NULL) {
            TRIM_NODE* pNode = m_pGlobalFreeListHead;
            m_pGlobalFreeListHead = pNode->m_pNextLPN;
            pNode->Reset();
...
VOID TRIM_MGR::Initialize(UINT32 clusterID)
{
    ...
    // [중요] 각 노드 초기화: m_nMetaLPN에는 Global Meta Index를 저장
    ...
    if (m_pCMTNodeMap != NULL) {
        for (UINT32 i = 0; i < nMetaPagesPerCluster; i++) {
            ...
            // GC가 Pop했을 때 LoadMeta에 바로 쓸 수 있도록 Global Index 저장
...
// [Helper] Hash Bucket 이동 함수
void TRIM_MGR::_UpdateCMTTrimCount(CMT_GROUP_NODE* pNode, INT32 nDelta)
{
    // [FIX] PopBestCMTGroup에서 이미 리스트를 끊었을 수 있으므로 확인 후 제거
    // list_empty가 FALSE(즉, 연결됨)일 때만 del 수행
    if (!list_empty(&pNode->m_dlHash)) {
        list_del(&pNode->m_dlHash);
    }
    // list_del_init이 되어있다면(Empty 상태), list_del을 스킵하고 바로 값만 갱신 후 재진입
    ...
    // 변경된 Count에 맞는 Bucket으로 이동
    list_add_tail(&pNode->m_dlHash, &m_ahTrimHash[nNewCount]);
}

// [수정] Global LPN -> Cluster Relative LPN -> Local Index 변환 및 Span 처리
void TRIM_MGR::IncreaseCMTCount(UINT32 nStartLPN, UINT32 nLength)
{
    ...
    // [안전장치] 클러스터 범위를 벗어나는 요청은 Clamp (매우 중요)
    ...
    // Cluster 범위 내의 상대 주소 (Relative LPN)
...
void TRIM_MGR::DecreaseCMTCount(UINT32 nStartLPN, UINT32 nLength)
{
    ...
    while (nRemain > 0)
    {
        ...
        // 0보다 클 때만 감소 (0이면 이미 GC 처리 완료되어 초기화된 상태일 수 있음)
...
CMT_GROUP_NODE* TRIM_MGR::PopBestCMTGroup()
{
    ...
    for (int i = CMT_BUCKET_SIZE - 1; i > 0; i--)
    {
        if (!list_empty(&m_ahTrimHash[i]))
        {
            ...
            // [FIX] Count를 0으로 초기화하지 않음!
            // ApplyPendingTrim에서 Fast Check(TotalTrim == 0)를 통과해야 하기 때문.
            // 대신 리스트에서만 안전하게 제거(list_del_init)하여 중복 선택 방지.
            // _UpdateCMTTrimCount(pBest, -((INT32)pBest->m_nTotalTrim)); <--- 삭제

            list_del_init(&pBest->m_dlHash);

            // 대기열에서 빠졌으므로 활성 그룹 수 명시적 감소
            if (m_nCMTGroupCount > 0) m_nCMTGroupCount--;
...
void TRIM_MGR::InsertTrim(UINT32 nStartLPN, UINT32 nLength)
{
    ...
    // 1. 메모리 부족 시 Fast Return (단일 노드 할당 불가 시)
    ...
    // [Step 2] 후방 병합 (Swallow Next Nodes)
    // pActive가 확장되면서 뒤쪽 노드(pNext)들과 겹치면 흡수
    ...
    while (pNext != NULL) {
        ...
        // *중요* 겹치는 구간은 pNext가 이미 카운트했었으므로, 중복 방지를 위해 차감(Decrease)
        ...
        // pNext가 pActive보다 더 길게 뻗어있었다면, pActive를 그만큼 더 확장
        // (확장된 부분에 대한 카운트는 pNext가 가지고 있었으므로 별도 Increase 불필요)
        ...
        pNext = pNext->m_pNextLPN; // 미리 포인터 이동

        _RemoveFromLists(pDel);

        // *중요* FreeNode() 대신 Raw Free 사용 (카운트 중복 차감 방지)
...
UINT32 TRIM_MGR::_GetLPNHash(UINT32 nLPN)
{
    ...
    // Cluster 범위 밖 LPN에 대한 방어 코드
    if (nLPN < nClusterBaseLPN) return 0;

	UINT32 nRelativeLPN = nLPN - nClusterBaseLPN;

    // Range Partitioning: 연속된 LPN들이 같은 Bucket에 들어가도록 함
...
// [핵심 기능] Meta Page 로드 시 지연된 TRIM 적용 (Overlap 처리)
BOOL TRIM_MGR::ApplyPendingTrim(UINT32 nMetaLPN, UINT32* pL2VBuffer)
{
    // 1. 정적 Map(CMTNodeMap)을 통해 Pending Trim이 있는지 빠르게 확인
    UINT32 nTotalLPN = DFTL_GLOBAL::GetInstance()->GetLPNCount();
    UINT32 nLPNsPerCluster = nTotalLPN / USER_CLUSTERS;
    UINT32 nBaseGlobalLPN = m_nClusterID * nLPNsPerCluster;
    UINT32 nBaseGlobalMetaLPN = nBaseGlobalLPN / L2V_PER_META_PAGE;
    ...
    for (UINT32 h = nStartHash; h <= nEndHash; h++)
    {
        ...
        while (pCurr != NULL)
        {
            TRIM_NODE* pNext = pCurr->m_pNextLPN; // 순회 중 삭제/변경 대비

            // 범위 체크
            UINT32 nNodeStart = pCurr->m_nStartLPN;
            UINT32 nNodeEnd = nNodeStart + pCurr->m_nLength;

            // 노드가 Meta Page 범위 뒤에 있으면 중단 (Sorted List이므로)
            if (nNodeStart >= nMetaEndLPN) break;

            // 노드가 Meta Page 범위 앞에 있으면 계속
            if (nNodeEnd <= nMetaStartLPN) {
                pCurr = pNext;
                continue;
            }

            // 4. Overlap 발생 -> TRIM 적용
            ...
            // 4-2. CMT Count 감소 (처리했으므로)
            // _UpdateCMTTrimCount 내부에서 !list_empty 체크하므로 안전함
            DecreaseCMTCount(nOverlapStart, nOverlapLen);
            if (m_nPendingTrimPages >= nOverlapLen) m_nPendingTrimPages -= nOverlapLen;

            // 4-3. Extent Node 조정 (Split, Shrink, Remove)

            // Case A: 노드가 Meta Page 범위 안에 완전히 포함됨 -> 노드 삭제
            if (nNodeStart >= nMetaStartLPN && nNodeEnd <= nMetaEndLPN) {
                _RemoveFromLists(pCurr);
                DFTL_GLOBAL::GetInstance()->FreeTrimNode(pCurr);
                if(m_nUsedNodeCount > 0) m_nUsedNodeCount--;
            }
            // Case B: 노드의 앞부분이 겹침 (뒷부분은 남음) -> Start 이동
            ...
            // Case C: 노드의 뒷부분이 겹침 (앞부분은 남음) -> Length 축소
            else if (nNodeStart < nMetaStartLPN && nNodeEnd <= nMetaEndLPN) {
                pCurr->m_nLength = nMetaStartLPN - nNodeStart;
            }
            // Case D: 노드가 Meta Page를 완전히 덮음 (중간만 구멍) -> Split
            else if (nNodeStart < nMetaStartLPN && nNodeEnd > nMetaEndLPN) {
                // 앞부분
...
void TRIM_MGR::InvalidateOverlap(UINT32 nWriteStart, UINT32 nWriteLength)
{
    ...
    // 시작 LPN이 MaxTrimLength보다 작을 경우 Underflow 방지
    ...
    for (UINT32 i = nStartBucketIdx; i <= nEndBucketIdx; i++)
    {
        ...
        while (pCurr != NULL)
        {
            // 리스트는 LPN 순으로 정렬되어 있으므로, 현재 노드의 시작점이 쓰기 끝지점보다 뒤면 더 볼 필요 없음
            if (pCurr->m_nStartLPN >= nWriteEnd) break;

            TRIM_NODE* pNext = pCurr->m_pNextLPN; // 노드 삭제/변경 대비 미리 저장
            UINT32 nTrimStart = pCurr->m_nStartLPN;
            UINT32 nTrimEnd = nTrimStart + pCurr->m_nLength;

            // 1. 겹치지 않는 경우 (쓰기 구간보다 노드가 뒤에 있음 - 위 break에서 걸러지지만 안전장치)
            //    또는 쓰기 구간보다 노드가 완전히 앞에 있는 경우
            ...
            // ---------------------------------------------------------
            // Case 1: Write가 Trim 노드를 완전히 덮어씀 (노드 삭제)
            // ---------------------------------------------------------
            if (nWriteStart <= nTrimStart && nWriteEnd >= nTrimEnd)
            {
                _RemoveFromLists(pCurr);
                FreeNode(pCurr); // FreeNode 내부에서 전체 길이만큼 DecreaseCMTCount 수행함
            }
            // ---------------------------------------------------------
            // Case 2: Write가 Trim 노드의 중간을 끊음 (Split)
            // ---------------------------------------------------------
            else if (nWriteStart > nTrimStart && nWriteEnd < nTrimEnd)
            {
                // [FIX] nWriteLength 만큼 감소시키되, 위치는 nWriteStart여야 함
                // 기존 오류: DecreaseCMTCount(pCurr->m_nStartLPN, nWriteLength);
            ...
            // ---------------------------------------------------------
            // Case 4: Write가 Trim 노드의 앞부분을 덮음 (Shrink Head)
            // ---------------------------------------------------------
            else
            {
                UINT32 nRemovedLen = nWriteEnd - nTrimStart;

                // [FIX] 겹치는 앞부분(nTrimStart == pCurr->m_nStartLPN)부터 감소
...
// [수정] 1:1 Map 변경으로 인해 Pool 함수 제거 또는 더미 처리
