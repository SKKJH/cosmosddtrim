...
// 우선순위 관리를 위한 구조체
struct ClusterSortInfo {
    UINT32 cid;
    UINT32 trimSize;
};

UINT32 ApplyTrimByPolicy_CMT(void)
{
    ...
    DFTL_GLOBAL* pstGlobal = DFTL_GLOBAL::GetInstance();
    META_MGR* pstMetaMgr = DFTL_GLOBAL::GetMetaMgr();
    ClusterSortInfo sortedClusters[USER_CLUSTERS];
    UINT32 validClusterCount = 0;
    for (UINT32 cid = 0; cid < USER_CLUSTERS; ++cid) {
        UINT32 size = pstGlobal->m_nTrimSize[cid];
        if (size > 0) {
            sortedClusters[validClusterCount].cid = cid;
            sortedClusters[validClusterCount].trimSize = size;
            validClusterCount++;
        }
    }
    for (UINT32 i = 0; i < validClusterCount; ++i) {
        for (UINT32 j = 0; j < validClusterCount - 1 - i; ++j) {
            if (sortedClusters[j].trimSize < sortedClusters[j+1].trimSize) {
                ClusterSortInfo temp = sortedClusters[j];
                sortedClusters[j] = sortedClusters[j+1];
    ...
    for (UINT32 i = 0; i < validClusterCount; ++i)
    {
        ...
        UINT32 bestCid = sortedClusters[i].cid;
        ...
        for (UINT32 segIdx = sBeg; segIdx <= sEnd; ++segIdx)
        {
            ...
            UINT16 cnt = g_trim_seg_count[segIdx];
            if (cnt < 256) continue;
            UINT32 startLPN = segIdx * LPNS_PER_SEGMENT;
            if (pstMetaMgr->ApplyTrimIfCached(startLPN) == TRUE) {	//0102
            	continue;
            }
            while (pstMetaMgr->IsMetaAvailable(startLPN, META_OPT) == FALSE)
            {
...
//        if (pstMetaMgr->IsMetaAvailable(startLPN, META_OPT) == TRUE) {	 //0102
//            continue;
//        }
        if (pstMetaMgr->ApplyTrimIfCached(startLPN) == TRUE) {		 //0102
        	continue;
