...
void DFTL_GLOBAL::UpdateTrimStat(UINT32 pages, UINT32 loads, UINT32 hits, UINT32 t_dma, UINT32 t_cmt, UINT32 t_wb, UINT32 t_l2v)
{
    TrimCallCount++;
    ...
    if (TrimCallCount == 1) {
        ...
        // 누적 평균 계산 로직
        AvgTrimPages += (INT32)(pages - AvgTrimPages) / (INT32)TrimCallCount;
        AvgCMTLoads  += (INT32)(loads - AvgCMTLoads)  / (INT32)TrimCallCount;
        AvgWBHits    += (INT32)(hits  - AvgWBHits)    / (INT32)TrimCallCount;
        AvgTimeDMA   += (INT32)(t_dma - AvgTimeDMA)   / (INT32)TrimCallCount;
        AvgTimeCMT   += (INT32)(t_cmt - AvgTimeCMT)   / (INT32)TrimCallCount;
        AvgTimeWB    += (INT32)(t_wb  - AvgTimeWB)    / (INT32)TrimCallCount;
        AvgTimeL2V   += (INT32)(t_l2v - AvgTimeL2V)   / (INT32)TrimCallCount;
    }
}

// 통계 출력 함수 구현
void DFTL_GLOBAL::PrintTrimStat(void)
{
    xil_printf("[TRIM STAT] Cnt:%u | AvgPg:%u | AvgLoad:%u | AvgWB:%u | AvgTime(DMA:%u CMT:%u WB:%u L2V:%u)\r\n",
            TrimCallCount, AvgTrimPages, AvgCMTLoads, AvgWBHits,
            AvgTimeDMA, AvgTimeCMT, AvgTimeWB, AvgTimeL2V
    );
}
