...
// [Updated] GC Logic: Prioritize 1GB TRIM Flush
BOOL
SUPER_GC_MGR::CheckAndStartGC()
{
    ...
    BOOL bTrimTrigger = TRUE;
    if (GC_BASE == 0) {
    	bTrimTrigger = (pstGlobal->m_nTotalPendingTrimCount >= TRIM_FLUSH_THRESHOLD);
    }
...
//		SBINFO_MGR* pstSBInfoMgr = DFTL_GLOBAL::GetSBInfoMgr();
//		processed = ApplyTrimByPolicy_TrimSize(TRIM_FLUSH_THRESHOLD, pstSBInfoMgr->m_pastSBInfo[CandVBN].m_bCID);
		...
		if (GC_BASE != 1)
		{
			if (GC_BASE == 2)
				...
				processed = ApplyTrimByPolicy_CMT();
		}
...
GC_MGR::IncreaseWriteCount(VOID)
{
	...
	if ((m_nWriteCount + m_nTrimSkipCount) >= m_nVPC) //0107
	{
		if(m_eIOType != IOTYPE_META) {
	...
	do
	{
		if (m_nCurReadVPageOffset >= pstGlobal->GetVPagePerVBlock())
		{
			if (m_eIOType != IOTYPE_META)
			{
				...
				{
					if (m_nVPC != m_nIssuedCount + m_nTrimSkipCount)
					{
						m_nVPC = m_nIssuedCount + m_nTrimSkipCount;
					}
				}
			}
			return;
		...
		if (bValid == TRUE)
		{
		    if (m_eIOType != IOTYPE_META && pstGlobal->m_nTotalPendingTrimCount != 0)
		    {
		        UINT32 candVPPN = VPPN_FROM_VBN_VPN(m_channel, m_way, m_nVictimVBN, m_nCurReadVPageOffset);
		        UINT32 lpn = pstVNand->GetV2L(candVPPN);

		        if (lpn != INVALID_LPN && lpn < (UINT32)TOTAL_LPN_COUNT)
		        {
		            UINT32 byte_idx = (lpn >> 3);
		            UINT32 bit_off  = (lpn & 0x7);

		            if (((g_trim_bitmap[byte_idx] >> bit_off) & 0x1u) != 0u)
		            {
		                ClearTrimRange(lpn, lpn);
		                pstVNand->Invalidate(candVPPN, 1);

		                VBINFO* vbi = DFTL_GLOBAL::GetVBInfoMgr(m_channel, m_way)->GetVBInfo(m_nVictimVBN);
		                vbi->IncreaseInvalidate();
		                vbi->DecreaseValidate();

		                m_nTrimSkipCount++;
		                xil_printf("	[GC] TRIM PASS LPN:%u, VPPN:%u\r\n", lpn, candVPPN);
		                continue;
		            }
		        }
		    }
