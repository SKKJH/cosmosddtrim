...
public:
	...
	VOID ClearBad(VOID)		{ m_bBad = FALSE;	}
	...
	VOID IncreaseValidate(VOID);
	VOID DecreaseValidate(VOID);
	BOOL IsFullInvalid(VOID);
	VOID SetFullInvalid(VOID);
	VOID SetInvalidPageCount(UINT32 nCount) { m_nInvalidLPN = nCount; }
	VOID SetValidPageCount(UINT32 nCount) { m_nValidLPN = nCount; }
	UINT32 GetInvalidLPNCount(VOID) const { return m_nInvalidLPN; }
	UINT32 GetValidLPNCount(VOID) const { return m_nValidLPN; }
...
public:
    ...
    VOID DebugPrintAllByVBN(UINT32 FLAG) const;
    VOID DebugPrintAllByVBN_CW(UINT32 channel, UINT32 way, UINT32 FLAG) const;

	VOID	Initialize(BLOCK_MGR_TYPE eType);
	VOID	Format(VOID);
	UINT32	Allocate(UINT32 channel, UINT32 way, BOOL bUser, BOOL bGC, BOOL bMeta, UINT32 nextVBN, UINT32 FLAG, UINT32 CID);				// get a free Virtual Block
	...
	VOID 	InvalidateForGC(UINT32 nVPPN);


	UINT32	CheckVPC(UINT32 channel, UINT32 way, UINT32 nVBN, UINT32 FLAG);
	UINT32 	VPC_Count(UINT32 channel, UINT32 way, UINT32 nVBN, UINT32 FLAG);
	VOID	INIT_CVPC(UINT32 channel, UINT32 way, UINT32 nVBN);

	struct list_head	m_dlUsedBlocks[USER_CHANNELS][USER_WAYS];		// allocated block list, 
...
private:
	...
	UINT32				m_nUsedBlocks[USER_CHANNELS][USER_WAYS];		// used block count
