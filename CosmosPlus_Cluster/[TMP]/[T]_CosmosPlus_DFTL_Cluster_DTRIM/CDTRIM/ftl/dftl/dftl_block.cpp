...
VOID BLOCK_MGR::_FormatMeta(VOID)
{
	...
	for (UINT32 channel = 0; channel < USER_CHANNELS; channel++)
	{
		for (UINT32 way = 0; way < USER_WAYS; way++)
		{
			...
			{
				nVBN = pstUserBlockMgr->Allocate(channel, way, TRUE, FALSE, FALSE, -1, 0, 0);
...
UINT32 BLOCK_MGR::Allocate(UINT32 channel, UINT32 way, BOOL bUser, BOOL bGC, BOOL bMeta, UINT32 nextVBN, UINT32 FLAG, UINT32 CID)
{
	if ((FLAG != 1) || (bMeta == 1))
	{
		...
		if(pstVBInfo->m_nVBN >= 20)
			xil_printf("1 [META] BLOCK ALLOC Something Wrong\r\n");
	...
	else
	{
	    ...
	    if (sb->m_nUSED == 0) {
	    	DFTL_GLOBAL::GetInstance()->m_util_blks[CID] += 1;
		...
		if(pstVBInfo->m_nVBN >= 4448)
		{
			if (bUser == TRUE)
				xil_printf("[HOST] %u BLOCK ALLOC Something Wrong\r\n", pstVBInfo->m_nVBN);
			else if (bMeta == TRUE)
				xil_printf("2 [META] %u BLOCK ALLOC Something Wrong\r\n", pstVBInfo->m_nVBN);
			else
				xil_printf("[GC] %u BLOCK ALLOC Something Wrong\r\n", pstVBInfo->m_nVBN);
		}
...
VOID BLOCK_MGR::Release(UINT32 channel, UINT32 way, UINT32 nVBN, UINT32 FLAG)
{
	...
	if ((FLAG == 1) && !pstVBInfo->IsMeta())
	{
	    ...
	    if (sb->m_nUSED == 0)
	    {
	    	DFTL_GLOBAL::GetInstance()->m_util_blks[sb->m_bCID] -= 1;
	    	sb->m_bCID = USER_CLUSTERS;
...
VOID BLOCK_MGR::InvalidateForGC(UINT32 nVPPN)
{
	UINT32 nVBN = VBN_FROM_VPPN(nVPPN);
	UINT32 channel = CHANNEL_FROM_VPPN(nVPPN);
	UINT32 way = WAY_FROM_VPPN(nVPPN);
