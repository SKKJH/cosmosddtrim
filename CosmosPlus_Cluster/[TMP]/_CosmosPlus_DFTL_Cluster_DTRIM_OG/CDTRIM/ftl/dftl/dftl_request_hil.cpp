...
HIL_REQUEST::_CheckMetaWritable(VOID)
{
   ...
   if (pstMetaMgr->IsMetaWritable(m_nLPN, 0) == FALSE) {
      return FALSE;
   }
   return TRUE;
#else
...
HIL_REQUEST::_ProcessRead_Wait(VOID)
{
   ...
   META_MGR*         pstMetaMgr        = DFTL_GLOBAL::GetMetaMgr();
   INT32  nVPPN   = pstMetaMgr->GetL2V(GetStartLPN() + m_nIssued_Count, 0);
...
//   last_wy = get_way_from_lpn(GetStartLPN() + m_nIssued_Count);
   ...
   for (int i = m_nIssued_Count; i < m_nLPNCount; i++) {
      ...
      nVPPN   = pstMetaMgr->GetL2V(nLPN, 0);
...
HIL_REQUEST_PER_WAY::_ProcessRead_Wait(VOID)
{
   ...
   for (int i = GetIssued_count(); i < GetLPNCount(); i++)
   {
      ...
      META_MGR* pstMetaMgr = DFTL_GLOBAL::GetMetaMgr();
      INT32 nVPPN   = pstMetaMgr->GetL2V(nLPN, 0);
      channel = CHANNEL_FROM_VPPN(nVPPN);
      way = WAY_FROM_VPPN(nVPPN);
//      channel = get_channel_from_lpn(nLPN);
//      way = get_way_from_lpn(nLPN);
      ...
      // GetPPN
      nVPPN = DFTL_GLOBAL::GetMetaMgr()->GetL2V(nLPN, 0);
    	...
    	for (UINT32 i = 0; i < m_nLPNCount; i++)
    	{
    		...
    		for (UINT32 j = 0; j < nLength; ++j, ++nCurLPN)
    		{
    			if (pstMetaMgr->IsMetaAvailable(nCurLPN, 0))
    			{
    				...
    				UINT32 nCurVPPN = pstMetaMgr->GetL2V(nCurLPN, 1);
