...
public:
    ...
    struct list_head 	m_dlHash;   // Hash KJH
...
public:
	VOID				Initialize(VOID);
	BOOL				Format(VOID);
	BOOL				IsMetaAvailable(UINT32 nLPN, UINT32 FLAG);
	BOOL				IsMetaWritable(UINT32 nLPN, UINT32 FLAG);
	META_CACHE_ENTRY*	GetMetaEntry(UINT32 nLPN, BOOL bIsGC);
	VOID				LoadMeta(UINT32 nLPN, UINT32 DS_FLAG, UINT32 META_FLAG);
	VOID 				DebugPrintLRUAndPos(META_CACHE_ENTRY* target);

	VOID                ReleaseConfinedEntries(VOID);

	UINT32	GetL2V(UINT32 nLPN, UINT32 FLAG);
	UINT32	SetL2V(UINT32 nLPN, UINT32 nVPPN, UINT32 FLAG);
...
public:
	...
	BOOL	Format(VOID);
	UINT32	GetL2V(UINT32 nLPN, UINT32 FLAG);
	VOID	SetL2V(UINT32 nLPN, UINT32 nVPPN, UINT32 DS_FLAG);
	...
	VOID 	ApplyTrimOnLoadedEntry(META_CACHE_ENTRY* pstMetaEntry, UINT32 metaLPN);
	VOID 	ApplyTrimOnLoadedEntry_OPT(META_CACHE_ENTRY* pstMetaEntry, UINT32 metaLPN);
	META_CACHE_ENTRY* GetMetaEntry(UINT32 nLPN, BOOL bIsGC) { return m_stMetaCache.GetMetaEntry(nLPN, bIsGC); }
