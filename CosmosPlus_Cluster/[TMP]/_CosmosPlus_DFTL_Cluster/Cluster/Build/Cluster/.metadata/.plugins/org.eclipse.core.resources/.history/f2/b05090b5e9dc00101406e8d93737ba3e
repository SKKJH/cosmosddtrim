...
//	}
//
//    if (bIsGC && m_nConfinedCount >= (META_CACHE_ENTRY_COUNT / 2))
//    {
...
//                do {
//                	pstRequest = pstRequestInfo->AllocateRequest();
//                	if (pstRequest == NULL) {
//                		DFTL_GLOBAL::GetInstance()->Run(); // 시스템을 돌리며 대기
//                	}
//                } while (pstRequest == NULL);
////                할당 성공 보장 (if (pstRequest != NULL) 체크 불필요)
//
//                if (pstRequest != NULL)
//                {
...
META_CACHE::LoadMeta(UINT32 nLPN, UINT32 DS_FLAG, BOOL bIsMETA)
{
...
#if (SUPPORT_META_DEMAND_LOADING == 1)
	// ----------------------------------------------------------------
	// 1. 캐시 히트 확인 (이미 있으면 리턴)
	// ----------------------------------------------------------------
	META_CACHE_ENTRY* pstEntry = GetMetaEntry(mod_lpn, FALSE);
	if (pstEntry != NULL)
	{
		return;
	}
	...
	// ----------------------------------------------------------------
	// 2. [수정] 캐시 엔트리 할당 대기 (Cache Full 방지)
	// ----------------------------------------------------------------
	pstEntry = NULL;
	do {
		pstEntry = _Allocate(); // 희생양을 찾아 할당 시도

		if (pstEntry == NULL)
		{
			// 캐시가 꽉 차고 모두 Busy 상태라면, I/O를 처리하며 빈자리가 생길 때까지 기다림
			FIL_Run();
			pstRequestMgr->Run();
		}
	} while (pstEntry == NULL); // 엔트리를 얻을 때까지 무한 대기
	...
	// ----------------------------------------------------------------
	// 3. [유지] 요청 객체 할당 대기 (Request Full 방지)
	// ----------------------------------------------------------------
	META_REQUEST* pstRequest = NULL;
	do {
		pstRequest = pstRequestInfo->AllocateRequest();
		...
		if (pstRequest == NULL)
		{
			// 요청 객체가 없다면, I/O를 처리하며 반납될 때까지 기다림
			FIL_Run();
			pstRequestMgr->Run();
		}
	} while (pstRequest == NULL); // 요청 객체를 얻을 때까지 무한 대기
...
#if (META_PER_WAY==1)
	UINT32 channel = get_channel_from_lpn(pstEntry->m_nMetaLPN);
	UINT32 way = get_way_from_lpn(pstEntry->m_nMetaLPN);
	pstRequestInfo->AddToWaitQ(pstRequest, channel, way);
#else
	pstRequestInfo->AddToWaitQ(pstRequest);
#endif

	pstEntry->m_bIORunning = TRUE; // I/O 중임 표시 (이것 때문에 _Allocate가 Busy로 판단함)

	if (DS_FLAG) DFTL_IncreaseProfile(Prof_Discard_Load_Num);
	META_DEBUG_PRINTF("[META] Load MetaLPN: %d \n\r", pstEntry->m_nMetaLPN);
