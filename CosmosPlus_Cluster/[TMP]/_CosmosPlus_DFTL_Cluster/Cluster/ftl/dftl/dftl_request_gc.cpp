...
//		xil_printf("	[SBN:%u] GC WRITE WAIT LPN:%u, CLUSTER ID:%u\r\n", SBN, m_nLPN, cID);
...
GC_REQUEST::_CheckAndLoadMeta(VOID)
{
	...
	if (pstMetaMgr->IsMetaAvailable(m_nLPN, TRUE) == FALSE)
	{
		pstMetaMgr->LoadMeta(m_nLPN, 0, TRUE);
		bAvailable = FALSE;
	}
...
GC_REQUEST::_isWatableMeta(VOID)
{
	...
	if (pstMetaMgr->IsMetaWritable(m_nLPN, TRUE) == FALSE)
	{
		bAvailable = FALSE;
	}
