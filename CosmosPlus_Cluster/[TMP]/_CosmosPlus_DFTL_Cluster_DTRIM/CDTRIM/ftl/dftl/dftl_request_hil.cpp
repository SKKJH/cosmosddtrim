...
HIL_REQUEST::_ProcessDSM_Wait(VOID)
{
    ...
    if (DFTL_GLOBAL::GetInstance()->m_bEnable > 1)
    {
    	for (UINT32 i = 0; i < m_nLPNCount; i++)
    	{
    		...
    		{
    			if (pstMetaMgr->IsMetaAvailable(nCurLPN, META_OPT))
    			{
    				...
    				UINT32 nCurVPPN = pstMetaMgr->GetL2V(nCurLPN, META_OPT);
    				if (nCurVPPN != INVALID_PPN) {
        				DFTL_IncreaseProfile(Prof_Discard_Page_Hit);
        	    		pstMetaMgr->SetL2V(nCurLPN, INVALID_PPN, 1, META_OPT);
    				}
    			}
    			else
    			{
