...
HIL_REQUEST::_CheckAndLoadMeta(VOID)
{
   ...
   {
      pstMetaMgr->LoadMeta(m_nLPN, 0);
      bAvailable = FALSE;
   }
   else if (pstMetaMgr->IsMetaAvailable(m_nLPN + m_nLPNCount - 1) == FALSE)
   {
      pstMetaMgr->LoadMeta(m_nLPN + m_nLPNCount - 1, 0);
      bAvailable = FALSE;
   }
...
HIL_REQUEST::_CheckLPNAndLoadMeta(UINT32 LPN)
{
   ...
   if (pstMetaMgr->IsMetaAvailable(LPN) == FALSE)
   {
      pstMetaMgr->LoadMeta(LPN, 0);
      bAvailable = FALSE;
   }
...
HIL_REQUEST::_ProcessDSM_Wait(VOID)
{
    ...
    META_MGR* pstMetaMgr = DFTL_GLOBAL::GetMetaMgr();
    BOOL bTrimEnabled = (DFTL_GLOBAL::GetInstance()->m_bEnable > 10);
    ...
    if (bTrimEnabled)
    {
    	...
    	for (UINT32 i = 0; i < m_nLPNCount; i++)
    	{
    	    ...
    	    for (UINT32 j = 0; j < nLength; ++j) {
                ck = false;
    	    	if (pstMetaMgr->IsMetaAvailable(nCurLPN) == FALSE)
    	    	...
    	    	while (pstMetaMgr->IsMetaAvailable(nCurLPN) == FALSE)
    	    	{
    	    		pstMetaMgr->LoadMeta(nCurLPN, 1);
