...
SUPER_GC_MGR::CheckAndStartGC()
{
    ...
    BOOL bTrimTrigger = (pstGlobal->m_nTotalPendingTrimCount >= TRIM_FLUSH_THRESHOLD);
	...
	if (bTrimTrigger)
	{
		...
		{
			DFTL_GLOBAL::GetInstance()->m_bEnable += 1;
		}

//		if (DFTL_GLOBAL::GetInstance()->m_bEnable == 3)
//			xil_printf("	[GC TRIM] Start\r\n");

		g_trim_going = 1;
		processed = ApplyTrimByPolicy_Util(TRIM_FLUSH_THRESHOLD);
//		processed = ApplyTrimByPolicy_TrimSize(TRIM_FLUSH_THRESHOLD);
	...
	if (CandVBN != INVALID_VBN)
	{
		if (processed > 0)
		{
			...
			{
//				xil_printf(" [GC-TRIM] Load CNT:%u, TRIM Size: %u, Reduced Pages %u(%u ~ %u)\r\n",
//						DFTL_GLOBAL::GetSuperGCMgr()->loadCNT, processed, prevVPC - victimVPC, prevVPC, victimVPC);
				DFTL_IncreaseProfile(Prof_DS_Reduced_Pages, prevVPC - victimVPC);
			}
//			else
//			{
//				xil_printf(" [GC-TRIM-R] Load CNT:%u, TRIM Size: %u, Reduced Pages %u(%u ~ %u)\r\n",
//						DFTL_GLOBAL::GetSuperGCMgr()->loadCNT, processed, prevVPC - victimVPC, prevVPC, victimVPC);
//			}
		}
...
GC_MGR::IncreaseWriteCount(VOID)
{
	...
	if (m_nWriteCount == m_nVPC)
	{
		...
		else
		{
			...
			{
//	            xil_printf("SUPER BLOCK[%u] GC END\r\n", DFTL_GLOBAL::GetSuperGCMgr()->m_nVictimVBN);
