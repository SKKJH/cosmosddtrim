...
//                    	xil_printf("[1] Increase Start %u ~ End %u\r\n", nStartLPN, nEndLPN);
        ...
        if (currentCID != INVALID_INDEX && pendingCountInCluster > 0) {
//        	xil_printf("[2] Increase Start %u ~ End %u\r\n", nStartLPN, nEndLPN);
            IncreaseTrimSize(currentCID, pendingCountInCluster);
        }
        if (changedCnt > 0) {
...
UINT32 ApplyTrimByPolicy_Util(UINT32 targetPageCount)
{
    ...
    const UINT32 PAGES_PER_SB    = 2048;
    const UINT32 LPNS_PER_CLUSTER = TOTAL_LPN_COUNT / USER_CLUSTERS; // 균등 분할 가정

    for (UINT32 i = 0; i < USER_CLUSTERS; ++i) {
        INT32 total_blks  = pstGlobal->m_util_blks[i];
        INT32 valid_pages = pstGlobal->m_util_pages[i];
        if (total_blks > 0) {
            g_clusterUtils[i] =
                (UINT32)((UINT64)valid_pages * 10000 / (total_blks * PAGES_PER_SB));
        } else {
            g_clusterUtils[i] = 0;
        }
    }
    ...
    for (UINT32 cid = 0; cid < USER_CLUSTERS; ++cid) {
        ...
        UINT32 cluster_lpn_end;

        if (cid == USER_CLUSTERS - 1) {
            // 마지막 클러스터: 나머지 LPN 전부 포함
            cluster_lpn_end = TOTAL_LPN_COUNT - 1;
        } else {
            cluster_lpn_end = (cid + 1) * LPNS_PER_CLUSTER - 1;
        }
        ...
        if (clusterSegEnd[cid] >= SEGMENT_COUNT) {
            clusterSegEnd[cid] = SEGMENT_COUNT - 1; // 방어용
        }
    }
    ...
    while (processedTotal < targetPageCount && remainingClusters > 0)
    {
        // 2-1. 아직 방문 안 한 클러스터 중 Util 가장 낮은 것 선택
        INT32  bestCid  = -1;
        UINT32 bestUtil = 0xFFFFFFFFu;

        for (UINT32 cid = 0; cid < USER_CLUSTERS; ++cid) {
            if (visited[cid]) continue;
            if (g_clusterUtils[cid] < bestUtil) {
                bestUtil = g_clusterUtils[cid];
                bestCid  = (INT32)cid;
            }
        }
        ...
        for (UINT32 segIdx = sBeg;
             segIdx <= sEnd && processedTotal < targetPageCount;
             ++segIdx)
        {
            ...
            if (pstMetaMgr->IsMetaAvailable(startLPN, 0) == FALSE) {
                while (pstMetaMgr->IsMetaAvailable(startLPN, 0) == FALSE)
                {
                    pstMetaMgr->LoadMeta(startLPN, 1, 1);
...
// [New] TRIM Size 기반 정책: TRIM이 많이 누적된 클러스터부터 처리
UINT32 ApplyTrimByPolicy_TrimSize(UINT32 targetPageCount)
{
    ...
    // 1. 클러스터별 LPN / 세그먼트 범위 계산 (Util 버전과 동일)
    UINT32 clusterSegStart[USER_CLUSTERS];
    UINT32 clusterSegEnd[USER_CLUSTERS];
    ...
    for (UINT32 cid = 0; cid < USER_CLUSTERS; ++cid) {
        ...
        if (clusterSegEnd[cid] >= SEGMENT_COUNT) {
            clusterSegEnd[cid] = SEGMENT_COUNT - 1; // 방어
        }
    }
    ...
    // 2. 클러스터 방문 여부 (TrimSize 많이 쌓인 순으로 방문)
    BOOL   visited[USER_CLUSTERS] = { 0 };
    UINT32 remainingClusters      = USER_CLUSTERS;

    while (processedTotal < targetPageCount && remainingClusters > 0u)
    {
        // 2-1. 아직 방문 안 한 클러스터 중 TRIM Size가 가장 큰 것 선택
        INT32  bestCid   = -1;
        UINT32 bestTrim  = 0;   // 0이면 의미 있는 TRIM 없음

        for (UINT32 cid = 0; cid < USER_CLUSTERS; ++cid) {
            if (visited[cid]) continue;

            UINT32 trimSize = pstGlobal->m_nTrimSize[cid];   // 클러스터별 대기 중 TRIM 양
            if (trimSize > bestTrim) {
                bestTrim = trimSize;
                bestCid  = (INT32)cid;
            }
        }

        // 남은 클러스터 중 TRIM이 하나도 없으면 종료
        if (bestCid < 0 || bestTrim == 0u) {
            break;
        }
             ...
             ++segIdx)
        {
            ...
            // 디버깅/제한용 (기존 Util 함수에서 쓰던 guard 그대로 둘 수도 있음)
            if (DFTL_GLOBAL::GetSuperGCMgr()->loadCNT == 40u) {
                return processedTotal;
            }
        }
