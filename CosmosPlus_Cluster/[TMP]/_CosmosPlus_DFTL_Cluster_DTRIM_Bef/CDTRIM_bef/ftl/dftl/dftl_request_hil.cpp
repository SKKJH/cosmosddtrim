...
//      {
//         UINT32 nCurHit  = DFTL_GetProfile(Prof_Host_CMT_Hit);
//         UINT32 nCurMiss = DFTL_GetProfile(Prof_Host_CMT_Miss);
//         UINT32 nCumulTotal = nCurHit + nCurMiss;
//
//         UINT32 nDeltaHit  = nCurHit - pGlobal->m_nLastHostHit;
//         UINT32 nDeltaMiss = nCurMiss - pGlobal->m_nLastHostMiss;
//         UINT32 nDeltaTotal = nDeltaHit + nDeltaMiss;
//
//         UINT32 nIntervalRatio = 0;
//         if (nDeltaTotal > 0)
//         {
//            nIntervalRatio = (nDeltaHit * 100) / nDeltaTotal;
//         }
//
//         UINT32 nCumulRatio = 0;
//         if (nCumulTotal > 0)
//         {
//            nCumulRatio = (nCurHit * 100) / nCumulTotal;
//         }
//
//         // 4. 출력 (Intv: 구간, Cumul: 누적)
//         // 예: [CMT Tracking] Req: 2000 | Intv: 90% (H:900 M:100) | Cumul: 85% (H:1700 M:300)
//
//         xil_printf("[CMT Tracking] Req: %u | Intv: %u%% (H:%u M:%u) | Cumul: %u%% (H:%u M:%u)\r\n",
//               pGlobal->m_nHostReqCount, nIntervalRatio, nDeltaHit, nDeltaMiss, nCumulRatio, nCurHit, nCurMiss);
//         pGlobal->m_nLastHostHit = nCurHit;
//         pGlobal->m_nLastHostMiss = nCurMiss;
//      }
//   }
}
...
HIL_REQUEST::_CheckAndLoadMeta(VOID)
{
   ...
   if (pstMetaMgr->IsMetaAvailable(m_nLPN, 1) == FALSE)
   {
      pstMetaMgr->LoadMeta(m_nLPN, 0, 0);
      bAvailable = FALSE;
   }
   else if (pstMetaMgr->IsMetaAvailable(m_nLPN + m_nLPNCount - 1, 1) == FALSE)
   {
      pstMetaMgr->LoadMeta(m_nLPN + m_nLPNCount - 1, 0, 0);
      bAvailable = FALSE;
   }
...
HIL_REQUEST::_CheckLPNAndLoadMeta(UINT32 LPN)
{
   ...
   if (pstMetaMgr->IsMetaAvailable(LPN, 1) == FALSE)
   {
      pstMetaMgr->LoadMeta(LPN, 0, 0);
      bAvailable = FALSE;
   }
...
HIL_REQUEST::_ProcessRead_Wait(VOID)
{
   ...
   META_MGR*         pstMetaMgr        = DFTL_GLOBAL::GetMetaMgr();
   INT32  nVPPN   = pstMetaMgr->GetL2V(GetStartLPN() + m_nIssued_Count);
...
HIL_REQUEST::_ProcessWrite_Wait(VOID)
{
   ...
   if (g_trim_pending)
   {
 	  ClearTrimRange(m_nLPN, m_nLPN + m_nLPNCount - 1);
   }
...
HIL_REQUEST_PER_WAY::_ProcessRead_Wait(VOID)
{
   ...
   for (int i = GetIssued_count(); i < GetLPNCount(); i++)
   {
      ...
      META_MGR* pstMetaMgr = DFTL_GLOBAL::GetMetaMgr();
      INT32 nVPPN   = pstMetaMgr->GetL2V(nLPN);
      channel = CHANNEL_FROM_VPPN(nVPPN);
      way = WAY_FROM_VPPN(nVPPN);
//      channel = get_channel_from_lpn(nLPN);
//      way = get_way_from_lpn(nLPN);
...
HIL_REQUEST::_ProcessDSM_Wait(VOID)
{
    ...
    pstHDMA->IssueRxDMA(
    		GetHostCmdSlotTag(),
			0,
			(UINT32)pBufferingAddr
    );
    SetHDMAIssueInfo(
    		pstHDMA->GetRxDMAIndex(),
			pstHDMA->GetRxDMAOverFlowCount()
    );
    pstHDMA->WaitRxDMADone(
    		m_nDMAReqTail,
			m_nDMAOverFlowCount
    );
    ...
    if (DFTL_GLOBAL::GetInstance()->m_bEnable > 1)
    {
    	...
    	for (UINT32 i = 0; i < m_nLPNCount; i++)
    	{
    		...
    		UINT32 nStartLPN = pDSMData[i * 4 + 2]; // 시작 LPN
    		UINT32 nEndLPN = nStartLPN + nLength - 1; // 끝 LPN

    		if (nLength == 0) continue;

    		UINT32 nCurLPN = nStartLPN;
    		UINT32 nTrimStartLPN = INVALID_LPN; // Trim이 필요한 LPN '구간'의 시작점을 기록

    		for (UINT32 j = 0; j < nLength; ++j, ++nCurLPN)
    		{
    			if (pstMetaMgr->IsMetaAvailable(nCurLPN, 0))
    			{
    				if (nTrimStartLPN != INVALID_LPN)
    				{
    					SetTrimRange(nTrimStartLPN, nCurLPN - 1);
    					nTrimStartLPN = INVALID_LPN;
    				}
    				UINT32 nCurVPPN = pstMetaMgr->GetL2V(nCurLPN);
    				if (nCurVPPN != INVALID_PPN) {
        				DFTL_IncreaseProfile(Prof_Discard_Page_Hit);
    					pstMetaMgr->SetL2V(nCurLPN, INVALID_PPN, 1);
    				}
    			}
    			else
    			{
    	...
    	DFTL_IncreaseProfile(Prof_Discard_Request_Num);

//		xil_printf("	[%u DISCARD] [LENGTH:%u] [Total Length:%u]\r\n",
//				DFTL_GLOBAL::GetInstance()->DS_CNT, DSLength, DFTL_GLOBAL::GetInstance()->DS_Length);
    }
