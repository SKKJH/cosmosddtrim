...
UINT32 SUPER_GC_MGR::ProcessSelectiveTRIM(VOID)
{
    ...
    const UINT32 CMT_LOAD_LIMIT = 30; // 최대 메타 로드 횟수 (예산)

    // -----------------------------------------------------------------------
    // 1. [Cluster Priority] 미래 유틸리티 기준 정렬
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // 2. [Execution] 정렬된 Cluster 순서대로 최적의 CMT 처리
    // -----------------------------------------------------------------------
    for (UINT32 k = 0; k < USER_CLUSTERS; k++)
    {
        ...
        // 해당 클러스터의 TRIM 작업 처리
        while (pTrimMgr->m_nCMTGroupCount > 0 && nCMTLoadCount < CMT_LOAD_LIMIT)
        {
            // [1] 처리할 최고의 CMT 그룹 획득 (Detach됨)
            CMT_GROUP_NODE* pBestGroup = pTrimMgr->PopBestCMTGroup();

            if (pBestGroup == NULL) break;

            UINT32 nMetaLPN = pBestGroup->m_nMetaLPN;
            UINT32 nBaseDataLPN = nMetaLPN * L2V_PER_META_PAGE;

            // ---------------------------------------------------
            // Case A: 메타 데이터 Hit (이미 로드됨)
            // ---------------------------------------------------
            if (pMetaMgr->IsMetaAvailable(nBaseDataLPN, TRIM_OPT))
            {
                META_CACHE_ENTRY* pEntry = pMetaMgr->GetMetaEntry(nBaseDataLPN, TRIM_OPT);
                if (pEntry) {
                    pMetaMgr->ApplyTrimOnLoadedEntry_OPT(pEntry, pEntry->m_nMetaLPN, TRIM_OPT);
                }

                // [필수] 사용 완료된 노드 해제
                pGlobal->FreeCMTGroupNode(pBestGroup);
            ...
            do {
                ...
                if (pstRequest != NULL) {
                    pstRequest->Run(); // 여기서 LoadDone 콜백 실행됨
                }

                if (pMetaMgr->IsMetaAvailable(nBaseDataLPN, TRIM_OPT)) break;

            } while (pstRequest != NULL);

            // [안전 장치 & 필수] 로드 완료 후 TRIM 적용 및 노드 해제
            // 콜백에서 수행되었더라도, 재진입 안전하게(Idempotent) 설계된 ApplyTrim을
            // 한 번 더 호출하여 확실하게 처리됨을 보장합니다.
            if (pMetaMgr->IsMetaAvailable(nBaseDataLPN, TRIM_OPT)) {
                 META_CACHE_ENTRY* pEntry = pMetaMgr->GetMetaEntry(nBaseDataLPN, TRIM_OPT);
                 if(pEntry) {
                     pMetaMgr->ApplyTrimOnLoadedEntry_OPT(pEntry, pEntry->m_nMetaLPN, TRIM_OPT);
            ...
            // [Fix] Miss Case에서도 반드시 노드를 해제해야 함 (Memory Leak 방지)
            pGlobal->FreeCMTGroupNode(pBestGroup);
        }
    }
...
UINT32 SUPER_GC_MGR::ProcessAllPendingTrim(VOID)
{
    ...
    for (UINT32 nCluster = 0; nCluster < USER_CLUSTERS; nCluster++)
    {
        ...
        // 무한 루프 방지용 (안전 장치)
        UINT32 nSafetyCount = 0;
        UINT32 nMaxLoop = pTrimMgr->m_nUsedNodeCount * 3 + 500;

        // 2. 해당 클러스터의 Pending Node가 모두 처리될 때까지 반복
        while (pTrimMgr->m_nUsedNodeCount > 0)
        {
            if (nSafetyCount++ > nMaxLoop)
            {
//                xil_printf("[GC] TRIM Stuck in Cluster %u. Force Cleaning %u nodes.\r\n",
//                            nCluster, pTrimMgr->m_nUsedNodeCount);
                // 강제 정리 (시스템 행 방지)
                for (int i = 0; i < TRIM_HASH_SIZE; i++) {
                    while (pTrimMgr->m_LPNHashTable[i]) {
                        TRIM_NODE* pDel = pTrimMgr->m_LPNHashTable[i];
                        pTrimMgr->_RemoveFromLists(pDel);
                        pTrimMgr->FreeNode(pDel);
                    }
                }
                break;
            }

            // 3. 타겟 노드 탐색 (존재하는 첫 번째 노드 선택)
            TRIM_NODE* pTargetNode = NULL;
            for (int i = 0; i < TRIM_HASH_SIZE; i++) {
                if (pTrimMgr->m_LPNHashTable[i] != NULL) {
                    pTargetNode = pTrimMgr->m_LPNHashTable[i];
                    break;
                }
            }

            if (pTargetNode == NULL) {
                pTrimMgr->m_nUsedNodeCount = 0; // 비정상 상태 보정
                break;
            }

            // 4. 타겟 노드가 속한 메타 LPN 확인
            UINT32 nLPN = pTargetNode->m_nStartLPN;

            // [Hit Case] 이미 로드되어 있는 경우
            // LoadMeta를 불러도 I/O가 발생하지 않으므로, 콜백(ApplyTrim)이 실행되지 않을 수 있음.
            // 따라서 이 경우에만 명시적으로 처리하여 노드를 제거함.
            if (pMetaMgr->IsMetaAvailable(nLPN, TRIM_OPT))
            {
                META_CACHE_ENTRY* pEntry = pMetaMgr->GetMetaEntry(nLPN, TRIM_OPT);
                if (pEntry != NULL)
                {
                    // 로드된 엔트리에 대해 TRIM 적용 (노드 삭제 포함)
                    pMetaMgr->ApplyTrimOnLoadedEntry_OPT(pEntry, pEntry->m_nMetaLPN, TRIM_OPT);
                    nProcessedCount++;
                }
                continue; // 다음 노드 처리로 이동
            }

            // [Miss Case] 메타 로드 요청 및 대기 (사용자 요청 로직)
            // 메타가 없으므로 로드 요청 -> I/O 완료 -> Callback -> ApplyTrim 자동 실행 -> 노드 삭제됨
            pMetaMgr->LoadMeta(nLPN, TRIM_OPT, TRIM_OPT); // 1: TRIM_OPT (가정)
            nProcessedCount++;

            META_REQUEST* pstRequest = NULL;
            do {
                // 1) 대기 큐 처리 및 낸드 시뮬레이션
                pReqMgr->_ProcessMetaRequestWaitQ();
                FIL_Run();

                // 2) 완료된 요청 확인 및 콜백 실행 (여기서 ApplyTrim이 호출됨)
                pstRequest = pMetaReqInfo->GetDoneRequest();
                if (pstRequest != NULL) {
                    pstRequest->Run();
                }

                // 3) 요청 처리 중 메타가 로드되었는지 확인 (빠른 탈출)
                // 로드가 완료되면 ApplyTrim에 의해 pTargetNode가 리스트에서 제거되었을 것임
                if (pMetaMgr->IsMetaAvailable(nLPN, TRIM_OPT)) break;

            } while (pstRequest != NULL); // 처리할 완료 요청이 있는 동안 반복
        }
    }

    // 5. 최종 검증 (Verification)
    // 모든 Pending Node가 해제되어 전역 Free Pool로 돌아왔는지 확인
    UINT32 nTotalFree = 0;
    TRIM_NODE* pCurr = pGlobal->m_pGlobalFreeListHead;
    while(pCurr) {
        nTotalFree++;
        pCurr = pCurr->m_pNextLPN;
    }

    // 또는 GlobalUsedNodeCount를 확인하는 것이 더 빠름
    if (pGlobal->m_nGlobalUsedNodeCount != 0)
    {
//        xil_printf("[GC] Warning: Pending Trim not fully cleared. Used: %d\r\n", pGlobal->m_nGlobalUsedNodeCount);
    }
    else
    {
//         xil_printf("[GC] All Pending Trim Processed Successfully.\r\n");
    }
...
GC_MGR::Run(VOID)
{
	...
	if (m_eIOType == IOTYPE_GC)
	{
		...
		{
//			xil_printf("		[WARN] META GC RUNNING\r\n");
			return;
		}
	}
#endif
...
//		if (pstGlobal->m_bEnable > 1)
//			xil_printf("META GC START [%u] FREE[%u], CH[%u], WY[%u]\r\n", m_nVictimVBN, nFreeBlock, m_channel, m_way);
	}
	else
	{
...
//				xil_printf("2 META GC END [%u] CH[%u], WY[%u]\r\n", pastVBN, m_channel, m_way);
		}
		else
		{
...
//		        BOOL bTrimmed = FALSE;
//
//		        // 1. TRIM 노드 검색 (MaxTrimLength 최적화 적용)
//		        if (pTrimMgr != NULL && pTrimMgr->m_nUsedNodeCount > 0)
//		        {
//		            // LPN이 포함될 수 있는 가장 앞선 StartLPN 계산
...
//		                while (pCurr != NULL)
//		                {
//		                    // 리스트가 StartLPN 오름차순이므로, 현재 노드 시작점이 nLPN보다 크면 중단 (Early Exit)
//		                    if (pCurr->m_nStartLPN > nLPN)
//		                    {
//		                        break;
//		                    }
//
//		                    // 포함 여부 확인 (Start <= nLPN < Start + Length)
...
//		        	META_CACHE_ENTRY* pEntry = NULL;
//
//		        	// [1] 메타 데이터 로드 및 포인터 획득 (Blocking Wait Loop)
//		        	// Loop 안에서 GetMetaEntry를 호출하여, 로드 성공 시 즉시 포인터를 가짐
//		        	while (TRUE)
//		        	{
//		        		pEntry = pstMetaMgr->GetMetaEntry(nLPN, TRIM_OPT);
//
//		        		// 로드 성공 시 루프 탈출
//		        		if (pEntry != NULL) {
//		        			break;
...
//
//		        		// 로드 요청
//		        		pstMetaMgr->LoadMeta(nLPN, TRIM_OPT, TRIM_OPT);
//
//		        		// I/O 구동
//		        		REQUEST_MGR* pstReqMgr = DFTL_GLOBAL::GetRequestMgr();
...
//		        			}
//
//		        			// 요청 하나 처리할 때마다 존재 여부 확인 (빠른 탈출)
//		        			// (여기서 pEntry를 갱신하지 않고, loop 위로 올라가서 GetMetaEntry 호출)
//		        			if (pstMetaMgr->IsMetaAvailable(nLPN, GC_OPT)) break;
//
//		        		} while (pstRequest != NULL);
//		        	}
//
//		        	// =================================================================
//		        	// [최적화] SetL2V 호출 없이 직접 업데이트 (Direct Update)
//		        	// =================================================================
//
//		        	// 1. L2V 테이블 값 직접 변경
//		        	UINT32 nOffset = nLPN % L2V_PER_META_PAGE;
//
//		        	// (방어 코드) 혹시 이미 INVALID면 통계 중복 감소 방지
//		        	if (pEntry->m_anL2V[nOffset] != INVALID_PPN)
//		        	{
//		        		pEntry->m_anL2V[nOffset] = INVALID_PPN;
//		        		pEntry->m_bDirty = TRUE; // [중요] Dirty 플래그 설정
//
//		        		// 2. 유효 페이지 카운트 수동 감소 (Cluster 통계)
//		        		UINT32 nClusterID = pstGlobal->GetClusterID(nLPN);
//		        		pstGlobal->m_util_pages[nClusterID] -= 1;
//
//		        		// 3. 물리적 무효화 (SetL2V가 하던 일을 여기서 직접 수행)
//		        		// GC 과정이므로 nVPPN은 현재 유효한 PPN임이 보장됨 (GetV2L 검증 생략 가능)
...
//		    // =========================================================
//
//		    break; // Valid하고 Trim되지 않은 페이지 -> 실제 Read 요청 수행
//		}
//		m_nCurReadVPageOffset++;
//
		...
		if (bValid == TRUE)
		{
		    ...
		    if (pstGlobal->IsValidLPN(nLPN))
		    {
		        ...
		        if (bTrimmed)
		        {
		        	...
		        	if (m_nVictimVBN == INVALID_VBN) return;
		        	m_nCurReadVPageOffset++;
		        	continue; // [핵심] Read/Write 없이 다음으로 건너뜀
		        }
		    }
		    // =========================================================
