...
META_CACHE::GetMetaEntry(UINT32 nLPN, BOOL bIsMETA)
{
	...
	if (pstEntry != NULL) {
		...
		nPrevLPN = nLPN;
		if (bIsMETA == FALSE) {
            DFTL_GLOBAL::GetInstance()->RecordHostAccess(TRUE);
		}
		return pstEntry;
	}

	if (bIsMETA == FALSE) {
		DFTL_GLOBAL::GetInstance()->RecordHostAccess(FALSE);
	}
...
META_CACHE::ReleaseConfinedEntries(VOID)
{
	...
	list_for_each_entry_safe(META_CACHE_ENTRY, e, temp, &m_dlConfinedLRU, m_dlConfinedList)
	{
        ...
        list_move_tail(&e->m_dlList, &m_dlLRU);
	}
    m_nConfinedCount = 0;
}
...
VOID
META_CACHE::LoadMeta(UINT32 nLPN, UINT32 DS_FLAG, BOOL bMetaFlag)
{
	BOOL bIsMETA;
	...
	// 1. 캐시에 이미 존재하는지 확인 (Hit)
	// GetMetaEntry 내부에서 Host/GC(bIsMETA) 여부에 따라 LRU 위치(Head/Tail) 조정이 처리됨
	META_CACHE_ENTRY* pstEntry = GetMetaEntry(mod_lpn, bIsMETA);
	if (pstEntry != NULL)
	{
		return;
	...
	// 2. GC 요청(bIsMETA)인 경우, Pinned(Confined) 엔트리 개수 제한 확인 및 공간 확보
	// PIN_LIMIT는 헤더에 정의된 값을 사용 (ex: 30%)
	if (bIsMETA && m_nConfinedCount > PIN_LIMIT)
	{
		META_CACHE_ENTRY* pVictim = NULL;
		struct list_head* pPos = m_dlConfinedLRU.prev; // 가장 오래된(Tail) Pinned 엔트리부터 탐색

		while (pPos != &m_dlConfinedLRU)
		{
			META_CACHE_ENTRY* pCand = list_entry(pPos, META_CACHE_ENTRY, m_dlConfinedList);
			// 현재 IO 작업 중이 아닌 엔트리만 축출 가능
		...
		if (pVictim != NULL)
		{
			if (pVictim->m_bDirty == TRUE)
			{
				...
				if (pstRequest != NULL)
				{
					pstRequest->Initialize(META_REQUEST_WRITE_WAIT, pVictim->m_nMetaLPN, pVictim, IOTYPE_META, 0);
				...
				// Write 요청을 보냈으므로 이번 Load는 다음 기회로 미룸 (혹은 IO 완료 대기)
				return;
			}
			else
			{
				// Clean 상태면 즉시 축출 (Evict)
				...
				pVictim->Initialize();
				_Release(pVictim); // Free List로 반환
	...
	// 4. GC 요청(bIsMETA)일 경우 Pin 설정 및 LRU 정책 적용
	if (bIsMETA) {
		pstEntry->m_bConfined = TRUE;
		list_add(&pstEntry->m_dlConfinedList, &m_dlConfinedLRU);
		m_nConfinedCount++;

		// [핵심] GC 데이터는 캐시 오염 방지를 위해 Main LRU의 Tail(끝)에 배치
	...
	pstRequest->Initialize(META_REQUEST_READ_WAIT, pstEntry->m_nMetaLPN, pstEntry, IOTYPE_META, bMetaFlag);
	...
	if (bMetaFlag == 2)
		DFTL_IncreaseProfile(Prof_GC_CMT_Load);
...
// [추가] Wrapper 함수 구현
META_CACHE_ENTRY* META_MGR::GetMetaEntry(UINT32 nLPN, UINT32 IsMETA)
{
#if (SUPPORT_META_DEMAND_LOADING == 1)
    return m_stMetaCache.GetMetaEntry(nLPN, IsMETA); // 0 = Host Mode (LRU Update)
#else
    return NULL;
#endif
}
