...
GC_REQUEST::_ProcessWrite_Wait(VOID)
{
	...
	else
	{
		...
		auto *ab = pstActiveBlock;
		static unsigned write_abnull_ctr = 0;
		if (!ab) {
		    write_abnull_ctr++;
		    if ((write_abnull_ctr & (write_abnull_ctr - 1)) == 0) {
		        xil_printf("[GC][WRITE] AB=NULL (pow2) cid=%u ch=%u wy=%u io=%u\n",
		               (unsigned)cID, (unsigned)ch, (unsigned)wy, (unsigned)m_eIOType);
		    }
		}
		...
		{
			static unsigned write_fail_ctr = 0;

		    write_fail_ctr++;
		    if ((write_fail_ctr % 1024) == 1) {
		        xil_printf("[GC][WRITE] WriteFail (every1024) cid=%u ch=%u wy=%u io=%u\n",
		               (unsigned)cID, (unsigned)ch, (unsigned)wy, (unsigned)m_eIOType);
		    }
		}
	}
...
GC_REQUEST::_CheckAndLoadMeta(VOID)
{
	...
	if (pstMetaMgr->IsMetaAvailable(m_nLPN, GC_OPT) == FALSE)
	{
		pstMetaMgr->LoadMeta(m_nLPN, GC_OPT, GC_OPT);
		bAvailable = FALSE;
		DFTL_IncreaseProfile(Prof_GC_CMT_Miss);
	}
	else
	{
		DFTL_IncreaseProfile(Prof_GC_CMT_Hit);
	}
...
GC_REQUEST::_isWatableMeta(VOID)
{
	...
	if (pstMetaMgr->IsMetaWritable(m_nLPN, GC_OPT) == FALSE)
	{
		bAvailable = FALSE;
	}
...
{
	static unsigned last_reason = 0xFFFFFFFF;
	unsigned reason = 0;
	...
	if (DFTL_GLOBAL::GetBufferMgr()->GetFreeCount() == 0)
	{
		...
		if (reason != last_reason) {
		    last_reason = reason;
		    xil_printf("[GC][WRITABLE] FALSE reason=%u ch=%u wy=%u\n",
		           reason, (unsigned)m_nChannel, (unsigned)m_nWay);
		}
