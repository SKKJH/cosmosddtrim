...
VOID DFTL_Profile_CMT_Initialize() {
	for (int iter = Prof_Host_CMT_Miss; iter < 2; iter++) {
		DFTL_profile[iter] = 0;
	}
}
...
VOID DFTL_PrintProfile(UINT32 FLAG)
{
	const char* profile_names[] = {
			...
			"Prof_DS_INCREASE_BIT",
			"Prof_DS_DECREASE_BIT",

			"Prof_DS_GC_Times",
			"Prof_DS_Discared_Pages",
	...
	xil_printf("PENDING TRIM CNT: %u\r\n", DFTL_GLOBAL::GetInstance()->m_nTotalPendingTrimCount);
...

	{
		...
		DFTL_GLOBAL::GetInstance()->VisualPrintAllTrimNodes();
	}
}
