...
VOID META_MGR::ApplyTrimOnLoadedEntry_OPT(META_CACHE_ENTRY* pEntry, UINT32 metaLPN, UINT32 FLAG)
{
    ...
    for (UINT32 i = nStartBucketIdx; i <= nEndBucketIdx; i++)
    {
        ...
        while (pCurr != NULL)
        {
            ...
            {
                pTrimMgr->_RemoveFromLists(pCurr);
                // FreeNode 내부에서 m_nPendingTrimPages 감소 및 DecreaseCMTCount 호출됨
                pTrimMgr->FreeNode(pCurr);
            }
            // Case 2: Split (노드가 메타 페이지 범위를 완전히 감쌈 -> 중간 삭제)
            else if (nStartLPN > nTrimStart && nEndLPN < nTrimEnd)
            {
                UINT32 nProcessedLen = nEndLPN - nStartLPN; // 처리된 중간 길이

                // [수정] 카운트 차감
                if (pTrimMgr->m_nPendingTrimPages >= nProcessedLen) pTrimMgr->m_nPendingTrimPages -= nProcessedLen;
                else pTrimMgr->m_nPendingTrimPages = 0;

                // [Fix] 삭제되는 구간(nStartLPN)을 기준으로 CMT 차감
                pTrimMgr->DecreaseCMTCount(nStartLPN, nProcessedLen);

                UINT32 nOldTrimEnd = nTrimEnd;

                // 2-1. 앞부분 수정 (StartLPN은 그대로, Length만 줄임)
                pTrimMgr->_UpdateNodeLengthOnly(pCurr, nStartLPN - nTrimStart);
                ...
                if (pSplit) {
                    ...
                    pTrimMgr->_AddToLPNList(pTrimMgr->_GetLPNHash(pSplit->m_nStartLPN), pSplit);
                    pTrimMgr->_AddToRangeList(pSplit);
                    // AllocNode는 메모리만 할당하므로, 원래 pCurr에 속했던 뒷부분에 대해 Increase 불필요
                }
                else {
                    // 할당 실패 시 뒷부분 유실 -> 추가 차감
                    UINT32 nLostLen = nOldTrimEnd - nEndLPN;
                    if (pTrimMgr->m_nPendingTrimPages >= nLostLen) pTrimMgr->m_nPendingTrimPages -= nLostLen;
                    else pTrimMgr->m_nPendingTrimPages = 0;

                    pTrimMgr->DecreaseCMTCount(nEndLPN, nLostLen);
                }
            }
            // Case 3: Shrink Head (노드가 메타 페이지 뒤쪽으로 걸침 -> 앞부분 삭제)
            else if (nStartLPN <= nTrimStart)
            {
                // 삭제되는 구간: [nTrimStart ~ nEndLPN]
                UINT32 nProcessedLen = nEndLPN - nTrimStart;

                // [수정] 카운트 차감
                if (pTrimMgr->m_nPendingTrimPages >= nProcessedLen) pTrimMgr->m_nPendingTrimPages -= nProcessedLen;
                else pTrimMgr->m_nPendingTrimPages = 0;

                // [Fix] 삭제되는 구간의 시작점(nTrimStart) 기준 차감
                pTrimMgr->DecreaseCMTCount(nTrimStart, nProcessedLen);
            ...
            {
                // 삭제되는 구간: [nStartLPN ~ nTrimEnd]
                UINT32 nProcessedLen = nTrimEnd - nStartLPN;

                // [수정] 카운트 차감
                if (pTrimMgr->m_nPendingTrimPages >= nProcessedLen) pTrimMgr->m_nPendingTrimPages -= nProcessedLen;
                else pTrimMgr->m_nPendingTrimPages = 0;

                // [Fix] 삭제되는 구간의 시작점(nStartLPN) 기준 차감
                pTrimMgr->DecreaseCMTCount(nStartLPN, nProcessedLen);
