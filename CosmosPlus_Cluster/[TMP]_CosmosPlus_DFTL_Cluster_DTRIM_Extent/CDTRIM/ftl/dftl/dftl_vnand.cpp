...
*/
VOID
VNAND::Invalidate(UINT32 nVPPN, UINT32 FLAG)
{
...
//		DFTL_GLOBAL::GetInstance()->nand_write_cnt++;

	if (HOST_CHECK == IOTYPE_HOST) {
		DFTL_IncreaseProfile(Prof_NAND_HOST_write);
//		if (DFTL_GLOBAL::GetInstance()->m_bEnable > 1)
//			DFTL_GLOBAL::GetInstance()->host_write_cnt++;
	}
...
//	unsigned int hw = DFTL_GLOBAL::GetInstance()->host_write_cnt;
//	if (DFTL_GLOBAL::GetInstance()->m_bEnable == 2)
//	{
//		if ((hw % 1000 == 0) && (hw > 1))
//		{
//			unsigned int nw = DFTL_GLOBAL::GetInstance()->nand_write_cnt;
//			unsigned int hw = DFTL_GLOBAL::GetInstance()->host_write_cnt;
//			unsigned long long waf_scaled = (unsigned long long)nw * 100;
//			unsigned int waf_val = (unsigned int)(waf_scaled / hw);
//			xil_printf("[WAF Info] HOST: %u, NAND: %u >> WAF : %u\r\n", hw, nw, waf_val);
//			DFTL_GLOBAL::GetInstance()->DS_CNT = 0;
//		}
//	}
}
