...
ACTIVE_BLOCK::Write(VOID* pstRequest, UINT32 cID, IOTYPE eRequestType, IOTYPE GC_TYPE)
{
	...
	else if (m_eBlockType == IOTYPE_GC)
	{
		...
		// Update Mapping
		DFTL_GLOBAL::GetMetaMgr()->SetL2V(pstGCRequest->GetLPN(), m_nCurVPPN, 0, TRUE);
...
//	}
	for (UINT32 i = 0; i < pstProgramUnit->m_nLPNCount; i++)
	{
		UINT32 nLPN = pstProgramUnit->GetLPN(i);
		if (nLPN != INVALID_LPN)
		{
...
ACTIVE_BLOCK_MGR::_GoToNextActiveBlock(IOTYPE eIOType, UINT32 nextVBN)
{
	...
	bool check = false;
	while (pstActiveBlock->CheckAllProgramUnitIsFree() == FALSE)
	{
		FIL_Run();
		if (!check)
			xil_printf("	RUN FREE\r\n");
	...
	if (pstActiveBlock->m_nVBN != INVALID_VBN)
	{
		...
		{
			pstBlockMgr->Release(m_nchannel, m_nway, pstActiveBlock->m_nVBN, 1);
