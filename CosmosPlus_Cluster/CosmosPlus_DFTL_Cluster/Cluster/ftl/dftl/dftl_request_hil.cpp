...
HIL_REQUEST::Run(VOID)
{
   ...
   case NVME_CMD_OPCODE_DSM:
      bSuccess = _ProcessDSM();
      break;
...
HIL_REQUEST::_CheckAndLoadMeta(VOID)
{
   ...
   if (pstMetaMgr->IsMetaAvailable(m_nLPN, FALSE) == FALSE)
   {
      pstMetaMgr->LoadMeta(m_nLPN, 0, FALSE);
      bAvailable = FALSE;
   }
   else if (pstMetaMgr->IsMetaAvailable(m_nLPN + m_nLPNCount - 1, FALSE) == FALSE)
   {
      pstMetaMgr->LoadMeta(m_nLPN + m_nLPNCount - 1, 0, FALSE);
      bAvailable = FALSE;
   }
...
BOOL
HIL_REQUEST::_CheckLPNAndLoadMeta(UINT32 LPN)
{
#if (SUPPORT_META_DEMAND_LOADING == 1)
   META_MGR*   pstMetaMgr = DFTL_GLOBAL::GetMetaMgr();
   ...
   if (pstMetaMgr->IsMetaAvailable(LPN, FALSE) == FALSE)
   {
      pstMetaMgr->LoadMeta(LPN, 0, FALSE);
      bAvailable = FALSE;
   }
...
HIL_REQUEST::_CheckMetaWritable(VOID)
{
   ...
   if (pstMetaMgr->IsMetaWritable(m_nLPN, FALSE) == FALSE) {
      return FALSE;
   }
   return TRUE;
#else
...
HIL_REQUEST::_ProcessRead_Wait(VOID)
{
   ...
   if (_CheckLPNAndLoadMeta(GetStartLPN() + m_nIssued_Count) == FALSE)
   {
      // load metadata
      return FALSE;
   }
   ...
   META_MGR*         pstMetaMgr        = DFTL_GLOBAL::GetMetaMgr();
   INT32  nVPPN   = pstMetaMgr->GetL2V(GetStartLPN() + m_nIssued_Count, FALSE);
   last_ch = CHANNEL_FROM_VPPN(nVPPN);
   last_wy = WAY_FROM_VPPN(nVPPN);
...
//   last_wy = get_way_from_lpn(GetStartLPN() + m_nIssued_Count);
   ...
   for (int i = m_nIssued_Count; i < m_nLPNCount; i++) {
      ...
      nLPN = GetStartLPN() + i;
      if (_CheckLPNAndLoadMeta(nLPN) == FALSE)
      {
         // load metadata
         return FALSE;
      }

      nVPPN   = pstMetaMgr->GetL2V(nLPN, FALSE);
...
BOOL
HIL_REQUEST::_ProcessDSM(VOID)
{
   BOOL   bSuccess;

   switch (m_nStatus)
   {
   case HIL_REQUEST_DSM_WAIT:
      bSuccess = _ProcessDSM_Wait();
      break;

//   case HIL_REQUEST_DSM_DONE:
//      bSuccess = _ProcessDSM_Done();
//      break;
...
HIL_REQUEST::_ProcessWrite_Wait(VOID)
{
   ...
   while (m_nIssued_Count < m_nLPNCount) {
	  UINT32 cID = DFTL_GLOBAL::GetInstance()->GetClusterID(m_nLPN + m_nIssued_Count);
      int tgt_ch = DFTL_GLOBAL::GetInstance()->m_wp_ch[cID];
      int tgt_wy = DFTL_GLOBAL::GetInstance()->m_wp_wy[cID];
      ...
      pstRequest_per_way->Initialize_per_way(
         HIL_REQUEST_WRITE_WAIT, NVME_CMD_OPCODE_WRITE,
         m_nLPN + m_nIssued_Count, m_nHostCmdSlotTag, 1, this);
      pstHILRequestInfo->AddToWaitQ_per_way(pstRequest_per_way, tgt_ch, tgt_wy);
      DFTL_GLOBAL::GetInstance()->WritePtr_GetAndAdvance(cID);
   ...
   if (m_nIssued_Count == m_nLPNCount)
   {
      ...
      HIL_REQUEST_INFO* pstHILRequestInfo2 = DFTL_GLOBAL::GetRequestMgr()->GetHILRequestInfo();
      pstHILRequestInfo2->RemoveFromWaitQ(this);
      pstHILRequestInfo2->AddToIssuedQ(this);
      return TRUE;
   }
   return FALSE;
}
...
HIL_REQUEST_PER_WAY::_ProcessRead_Wait(VOID)
{
   ...
   for (int i = GetIssued_count(); i < GetLPNCount(); i++)
   {
      ...
      META_MGR* pstMetaMgr = DFTL_GLOBAL::GetMetaMgr();
      INT32 nVPPN   = pstMetaMgr->GetL2V(nLPN, FALSE);
      channel = CHANNEL_FROM_VPPN(nVPPN);
      way = WAY_FROM_VPPN(nVPPN);
//      channel = get_channel_from_lpn(nLPN);
//      way = get_way_from_lpn(nLPN);
      ...
      // GetPPN
      nVPPN = DFTL_GLOBAL::GetMetaMgr()->GetL2V(nLPN, FALSE);
...
skip_readbuf:
         ...
         nBufferingVPPN = pstBufferingLPN->ReadLPN(nLPN, pBufEntry[i], cID, channel, way);
         ...
         if (nBufferingVPPN != INVALID_VPPN)
         {
            ...
            pBufEntry[i]->nVPPN = nVPPN;
            ReadCacheBuf = DFTL_GLOBAL::GetReadCacheMgr()->change_next_buffer(nLPN >> NUM_BIT_LPN_PER_PAGE, pBufEntry[i], channel, way);
            if (ReadCacheBuf == NULL)
               return false;
            IncreaseDoneCount();
...
HIL_REQUEST_PER_WAY::_ProcessWrite_Wait(VOID)
{
   ...
   do
   {
      ...
      UINT32 cID = DFTL_GLOBAL::GetInstance()->GetClusterID(GetCurLPN());
      pstActiveBlock = DFTL_GLOBAL::GetActiveBlockMgr(cID, m_channel, m_way)->GetActiveBlock(IOTYPE_HOST);
      ...
      bSuccess = pstActiveBlock->Write(this, cID, IOTYPE_HOST, IOTYPE_HOST);
...
//static UINT32 g_TrimCallCount   = 0; // 호출 횟수
//static UINT32 g_AvgTrimPages    = 0; // 평균 페이지 수
//static UINT32 g_AvgCMTLoads     = 0; // 평균 CMT Load 횟수
//static UINT32 g_AvgWBHits       = 0; // [추가] 평균 WB 접근(Hit) 횟수
//static UINT32 g_AvgTimeDMA      = 0; // 평균 DMA 시간
//static UINT32 g_AvgTimeCMT      = 0; // 평균 CMT Load 시간
//static UINT32 g_AvgTimeWB       = 0; // 평균 WB Invalidate 시간
//static UINT32 g_AvgTimeL2V      = 0; // 평균 L2V Update 시간

BOOL
HIL_REQUEST::_ProcessDSM_Wait(VOID)
{
    XTime tStart, tEnd;
    UINT32 t_dma = 0;
    UINT32 t_cmt = 0;
    ...
    UINT32 currentCmdPageCount = 0;
    UINT32 currentCmdLoadCount = 0;
    UINT32 currentCmdWBHitCount = 0;

    UINT32 DSLength = 0;
    ASSERT(m_nStatus == HIL_REQUEST_DSM_WAIT);
    REQUEST_MGR* pstRequestMgr = DFTL_GLOBAL::GetRequestMgr();
    BUFFER_MGR* pstBufferMgr  = DFTL_GLOBAL::GetBufferMgr();
    HDMA* pstHDMA       = DFTL_GLOBAL::GetHDMAMgr();

    INIT_LIST_HEAD(&m_dlBuffer);
    BUFFER_ENTRY* m_pstBufferEntry = pstBufferMgr->Allocate();
    list_add_tail(&m_pstBufferEntry->m_dlList, &m_dlBuffer);
    void* pBufferingAddr = m_pstBufferEntry->m_pMainBuf;

    XTime_GetTime(&tStart);
    pstHDMA->IssueRxDMA(GetHostCmdSlotTag(), 0, (UINT32)pBufferingAddr);
    SetHDMAIssueInfo(pstHDMA->GetRxDMAIndex(), pstHDMA->GetRxDMAOverFlowCount());
    pstHDMA->WaitRxDMADone(m_nDMAReqTail, m_nDMAOverFlowCount);
    XTime_GetTime(&tEnd);
    t_dma += (UINT32)(tEnd - tStart);

    UINT32* pDSMData = (UINT32*)pBufferingAddr;
    META_MGR* pstMetaMgr = DFTL_GLOBAL::GetMetaMgr();
    BOOL bTrimEnabled = (DFTL_GLOBAL::GetInstance()->m_bEnable > 5);

    if (bTrimEnabled)
    {
        for (UINT32 i = 0; i < m_nLPNCount; i++)
        {
            UINT32 nLength = pDSMData[i * 4 + 1];
            UINT32 nCurLPN = pDSMData[i * 4 + 2];

            if (nLength == 0) continue;
            currentCmdPageCount += nLength;

            for (UINT32 j = 0; j < nLength; ++j) {
                // Hit/Miss 확인
                if (pstMetaMgr->IsMetaAvailable(nCurLPN, TRUE) == FALSE) {
                    DFTL_IncreaseProfile(Prof_Discard_Page_Miss);
                    currentCmdLoadCount++;
                } else {
                ...
                while (pstMetaMgr->IsMetaAvailable(nCurLPN, TRUE) == FALSE)
                {
                    pstMetaMgr->LoadMeta(nCurLPN, 1, TRUE);
                ...
                if(ck) {
                    XTime_GetTime(&tEnd);
                    t_cmt += (UINT32)(tEnd - tStart);
                }

                UINT32 nCurVPPN = pstMetaMgr->GetL2V(nCurLPN, TRUE);
                if (nCurVPPN != INVALID_PPN) {
                    UINT32 channel = CHANNEL_FROM_VPPN(nCurVPPN);
                    UINT32 way = WAY_FROM_VPPN(nCurVPPN);
                    UINT32 cID = DFTL_GLOBAL::GetInstance()->GetClusterID(nCurLPN);

                    ACTIVE_BLOCK_MGR* pstActiveBlockMgr = DFTL_GLOBAL::GetActiveBlockMgr(cID, channel, way);
                    if (pstActiveBlockMgr) {
                        ACTIVE_BLOCK* pstActiveBlock = pstActiveBlockMgr->GetActiveBlock(IOTYPE_HOST);
                        if (pstActiveBlock) {
                            currentCmdWBHitCount++;
                            XTime_GetTime(&tStart);
                            pstActiveBlock->InvalidateLPN(nCurLPN);
                            XTime_GetTime(&tEnd);
                            t_wb += (UINT32)(tEnd - tStart);
                        }
                    }

                    // 4. Set L2V 시간
                    XTime_GetTime(&tStart);
                    pstMetaMgr->SetL2V(nCurLPN, INVALID_PPN, 1, TRUE);
                    XTime_GetTime(&tEnd);
                    t_l2v += (UINT32)(tEnd - tStart);

                    DFTL_IncreaseProfile(Prof_Discard_Page_Num);
                } else {
                    DFTL_IncreaseProfile(Prof_Discard_Pass_Num);
                }

                nCurLPN++;
                DFTL_IncreaseProfile(Prof_Discard_Total_Page_Num);
                DFTL_GLOBAL::GetMetaMgr()->ReleaseConfinedEntries();
            }
            DFTL_IncreaseProfile(Prof_Discard_Range_Num);
        }
        if (DFTL_GLOBAL::GetInstance()->DS_CNT == 0) {
            DFTL_GLOBAL::GetInstance()->DS_CNT = 1;
        }
    }
    pstBufferMgr->Release(m_pstBufferEntry);
    GoToNextStatus();
...
//        g_TrimCallCount++;
//
//        if (g_TrimCallCount == 1) {
//            g_AvgTrimPages = currentCmdPageCount;
//            g_AvgCMTLoads  = currentCmdLoadCount;
//            g_AvgWBHits    = currentCmdWBHitCount; // [초기화]
//            g_AvgTimeDMA   = t_dma;
//            g_AvgTimeCMT   = t_cmt;
//            g_AvgTimeWB    = t_wb;
//            g_AvgTimeL2V   = t_l2v;
//        } else {
//            // 점진적 평균 계산
//            g_AvgTrimPages += (INT32)(currentCmdPageCount - g_AvgTrimPages) / (INT32)g_TrimCallCount;
//            g_AvgCMTLoads  += (INT32)(currentCmdLoadCount - g_AvgCMTLoads)  / (INT32)g_TrimCallCount;
//            g_AvgWBHits    += (INT32)(currentCmdWBHitCount - g_AvgWBHits)  / (INT32)g_TrimCallCount; // [갱신]
//            g_AvgTimeDMA   += (INT32)(t_dma - g_AvgTimeDMA)                 / (INT32)g_TrimCallCount;
//            g_AvgTimeCMT   += (INT32)(t_cmt - g_AvgTimeCMT)                 / (INT32)g_TrimCallCount;
//            g_AvgTimeWB    += (INT32)(t_wb  - g_AvgTimeWB)                  / (INT32)g_TrimCallCount;
//            g_AvgTimeL2V   += (INT32)(t_l2v - g_AvgTimeL2V)                 / (INT32)g_TrimCallCount;
//        }
//
//        // 결과 출력 (AvgWB 항목 추가)
//        xil_printf("[TRIM STAT] Cnt:%u | AvgPg:%u | AvgLoad:%u | AvgWB:%u | AvgTime(DMA:%u CMT:%u WB:%u L2V:%u)\r\n",
//            g_TrimCallCount,
//            g_AvgTrimPages,
//            g_AvgCMTLoads,
//            g_AvgWBHits,
...
//BOOL
//HIL_REQUEST::_ProcessDSM_Done(VOID)
//{
   // Release Request
   //Remove from DoneQ
//   REQUEST_MGR*   pstRequestMgr = DFTL_GLOBAL::GetRequestMgr();
...
//   pstHILRequestInfo->ReleaseRequest(this);
//   xil_printf("[DSM] _ProcessDSM_Done");
//
//   return TRUE;
//}
