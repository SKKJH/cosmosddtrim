...
typedef struct
{
	...
	unsigned int anVerificationSpare[USER_CHANNELS][USER_WAYS][TOTAL_BLOCKS_PER_DIE][ROWS_PER_MLC_BLOCK][LPN_PER_PHYSICAL_PAGE];			// 4 BYTE VERIFICATION SPARE
...
static unsigned int _GetRowAddress(int nPCh, int nPWay, int nBlock, int nPage, int func);
...
NAND_RESULT NAND_IssueErase(int nPCh, int nPWay, int nBlock, int bSync)
{
	...
	//xil_printf("[NAND] Erase - Ch/Way/Block:%d/%d/%d\n\r", nPCh, nPWay, nBlock);
...
NAND_RESULT NAND_ProcessErase(int nPCh, int nPWay, int nBlock, int bSync)
{
	...
	do
	{
		...
		case NAND_ERASE_STEP_ISSUE_ERASE:
			nRowAddr = _GetRowAddress(nPCh, nPWay, nBlock, 0 /*Don't Care */, 0);
		...
		case NAND_ERASE_STEP_CHECK_STATUS:
			pnStatusResult = (volatile unsigned int*)&g_pstNAND->pstNAND_Status->anData[nPCh][nPWay];
			if (V2FRequestReportDone(*pnStatusResult))
			{
				volatile unsigned int nStatus = V2FEliminateReportDoneFlag(*pnStatusResult);
	...
	if (nResult == NAND_RESULT_DONE)
	{
		...
		{
			OSAL_MEMSET(&g_pstNAND->pstDebug->anVerificationSpare[nPCh][nPWay][nBlock], 0xFF, sizeof(unsigned int) * ROWS_PER_MLC_BLOCK * LPN_PER_PHYSICAL_PAGE);
...
NAND_RESULT NAND_IssueRead(int nPCh, int nPWay, int nBlock, int nPage, void* pMainBuf, void* pSpareBuf, int bECCOn, int bSync)
{
	...
	//xil_printf("[NAND] Read - Ch/Way/Block/Page/bECCOn:%d/%d/%d/%d/%d\n\r", nPCh, nPWay, nBlock, nPage, bECCOn);
...
NAND_RESULT NAND_ProcessRead(int nPCh, int nPWay, int nBlock, int nPage, void* pMainBuf, void* pSpareBuf, int bECCOn, int bSync)
{
	...
	do
	{
		...
		case NAND_READ_STEP_ISSUE_READ:
			nRowAddr = _GetRowAddress(nPCh, nPWay, nBlock, nPage, 1);
		...
		case NAND_READ_STEP_CHECK_READ_RESULT:
			...
			if (V2FRequestReportDone(*pnStatusResult))
			{
				...
				if (V2FRequestComplete(nStatus))
				{
					if (V2FRequestFail(nStatus))
					{
						if(g_pstNAND->stWayStatus[nPCh][nPWay].nRetry == 0)
						// Fail
						xil_printf("NAND Read Fail - Ch/Way/Block/Page:%d/%d/%d/%d, Try:%d\r\n", nPCh, nPWay, nBlock, nPage, g_pstNAND->stWayStatus[nPCh][nPWay].nRetry);
		...
		case NAND_READ_STEP_ISSUE_TRANSFER:
			...
			if (bECCOn == TRUE)
			{
				nRowAddr = _GetRowAddress(nPCh, nPWay, nBlock, nPage, 2);
...
//				{
//					xil_printf("NAND Read Fail UECC - Ch/Way/Block/Page:%d/%d/%d/%d, Error0/Error1:0x%X/0x%X, Try:%d\r\n",
...
//			}
				...
				if ((bECCOn == TRUE) && (_GetTransferResult(panErrorInfo) == ERROR_INFO_FAIL))
				{
					xil_printf("[WARNING] UECC Ignored! Returning DUMMY - Ch:%d W:%d Blk:%d Pg:%d\r\n", nPCh, nPWay, nBlock, nPage);
					if (pMainBuf != NULL)
					{
						OSAL_MEMSET(pMainBuf, 0xFF, LOGICAL_PAGE_SIZE * LPN_PER_PHYSICAL_PAGE);
					}
					if (pSpareBuf != NULL)
					{
						OSAL_MEMSET(pSpareBuf, 0xFF, BYTES_PER_SPARE_REGION_OF_PAGE);
					}
					*pnCurStatus = NAND_READ_STEP_DONE;
				}
				else
		...
		if (*pnCurStatus == NAND_READ_STEP_DONE)
		{
			...
			*pnCurStatus = NAND_READ_STEP_NONE;
			...
			{
				((unsigned int*)pSpareBuf)[i]	= g_pstNAND->pstDebug->anVerificationSpare[nPCh][nPWay][nBlock][nPage][i];
...
NAND_RESULT NAND_IssueProgram(int nPCh, int nPWay, int nBlock, int nPage, void* pMainBuf, void* pSpareBuf, int bSync)
{
	...
	//xil_printf("[NAND] Program - Ch/Way/Block/Page:%d/%d/%d/%d\n\r", nPCh, nPWay, nBlock, nPage);
...
NAND_RESULT NAND_ProcessProgram(int nPCh, int nPWay, int nBlock, int nPage, void* pMainBuf, void* pSpareBuf, int bSync)
{
	...
	do
	{
		...
		case NAND_PGM_STEP_ISSUE_PROGRAM:
			nRowAddr = _GetRowAddress(nPCh, nPWay, nBlock, nPage, 3);
		...
		case NAND_PGM_STEP_CHECK_STATUS:
			...
			if (V2FRequestReportDone(*pnStatusResult))
			{
				...
				if (V2FRequestComplete(nStatus))
				{
					if (V2FRequestFail(nStatus))
					{
						xil_printf("NAND Program Fail - Ch/Way/Block/Page:%d/%d/%d/%d\r\n", nPCh, nPWay, nBlock, nPage);
					}
					else
					{
						// Success
		...
		if (*pnCurStatus == NAND_PGM_STEP_DONE)
		{
			...
			*pnCurStatus = NAND_PGM_STEP_NONE;
			...
			for (int i = 0; i < LPN_PER_PHYSICAL_PAGE; i++)
			{
				...
				g_pstNAND->pstDebug->anVerificationSpare[nPCh][nPWay][nBlock][nPage][i] = ((unsigned int*)pSpareBuf)[i];

				//for IOUB modify
				g_pstNAND->pstDebug->anVerificationData[nPCh][nPWay][nBlock][nPage][i] = ((unsigned int*)pMainBuf)[i * HOST_BLOCK_SIZE / sizeof(unsigned int*)];
			}
...
static unsigned int _GetRowAddress(int nPCh, int nPWay, int nBlock, int nPage, int func)
{
	...
	if(nLUN > 1) {
		xil_printf("nLUN:%u, CH %d WY %d  BLK %d PG %d Func%d\r\n", nLUN, nPCh, nPWay, nBlock, nPage, func);
	}
	ASSERT(nLUN <= 1);
