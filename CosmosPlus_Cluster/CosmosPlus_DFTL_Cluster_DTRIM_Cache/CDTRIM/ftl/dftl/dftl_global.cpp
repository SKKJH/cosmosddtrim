...
UINT32 ApplyTrimByPriority(UINT32 targetPageCount, UINT32* pCids, UINT32 nCount)
{
    if (targetPageCount == 0 || nCount == 0) return 0;
    ...
    for (UINT32 i = 0; i < nCount; ++i)
    {
        ...
        if (cid >= USER_CLUSTERS || pstGlobal->m_nTrimSize[cid] == 0) continue;
        ...
        {
            if (processedTotal >= targetPageCount) return processedTotal;
            if (DFTL_GLOBAL::GetSuperGCMgr()->loadCNT >= targetPageCount) return processedTotal;

            if (g_trim_seg_count[segIdx] < 256) continue;

            UINT32 startLPN = segIdx * LPNS_PER_SEGMENT;

            if (pstMetaMgr->ApplyTrimIfCached(startLPN) == TRUE) {
                continue;
            }

            while (pstMetaMgr->IsMetaAvailable(startLPN, TRIM_OPT) == FALSE)
            {
                pstMetaMgr->LoadMeta(startLPN, 1, TRIM_OPT);
