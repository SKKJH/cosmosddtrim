...
#define META_REQUEST_COUNT			(USER_CHANNELS * USER_WAYS) * 30
...
public:
	...
	VOID	Initialize(META_REQUEST_STATUS nStatus, UINT32 m_nMetaLPN, VOID* pstMetaEntry, IOTYPE eIOType, UINT32 DS_FLAG);
	BOOL	Run(VOID);
...
#if (META_PER_WAY==1)
	VOID		AddToWaitQ(META_REQUEST* pstRequest, UINT32 channel, UINT32 way);
#else
...
#if (META_PER_WAY==1)
	VOID		RemoveFromWaitQ(META_REQUEST* pstRequest, UINT32 channel, UINT32 way);
#else
...
#if (META_PER_WAY==1)
	META_REQUEST*	GetWaitRequest(UINT32 channel, UINT32 way);
#else
...
#if (META_PER_WAY==1)
	struct list_head		m_dlWait[USER_CHANNELS][USER_WAYS];				// NAND issue wait queue
	INT32					m_nWaitCount[USER_CHANNELS][USER_WAYS];
#else
...
public:
	INT32 GetFreeCount(VOID) { return m_nFreeCount; }
};
