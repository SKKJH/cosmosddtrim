...
{
	ASSERT(m_nCurVPPN != INVALID_VPPN);
	ASSERT(VBN_FROM_VPPN(m_nCurVPPN) == m_nVBN);

	PROGRAM_UNIT*	pstProgramUnit = &m_astBuffering[m_nCurProgramBuffering];

	if (pstProgramUnit->m_bProgramming == TRUE)
	{
		ASSERT(pstProgramUnit->m_bFree == FALSE);
		ASSERT(pstProgramUnit->m_nLPNCount == LPN_PER_PHYSICAL_PAGE);
...
ACTIVE_BLOCK_MGR::_GoToNextActiveBlock(IOTYPE eIOType)
{
	...
	VBINFO* pstVBInfo = DFTL_GLOBAL::GetVBInfoMgr(m_nchannel, m_nway)->GetVBInfo(nVBN);
...
//			xil_printf("	targetVPPN: %u, nVPPN: %u\n\r", targetVPPN, nVPPN);
