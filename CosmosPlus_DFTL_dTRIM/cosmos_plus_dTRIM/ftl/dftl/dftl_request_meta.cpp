...
VOID
META_REQUEST::Initialize(META_REQUEST_STATUS nStatus, UINT32 nMetaLPN, VOID* pstMetaEntry, IOTYPE eIOType, UINT32 DS_FLAG)
{
	UINT32 channel, way;
...
META_REQUEST::_ProcessRead_Done(VOID)
{
	...
	pstMetaMgr->LoadDone(pstMetaEntry, (VOID*)nBufAddr);
...
META_REQUEST::_ProcessWrite_Wait(VOID)
{
	...
	pstActiveBlock = DFTL_GLOBAL::GetActiveBlockMgr(channel, way)->GetActiveBlock(IOTYPE_META);
	bSuccess = pstActiveBlock->Write(this, IOTYPE_META, IOTYPE_META);
	if (bSuccess == TRUE)
	{
		GoToNextStatus();
