...
static inline UINT16 trim_dense_threshold()
{
	const UINT32 SEGMENT_LPN = 1024u;
    return 800;

//    return (UINT16)(SEGMENT_LPN / 2u); // 50%
}


static inline bool is_dense_segment(UINT32 seg)
{
    return g_trim_seg_count[seg] >= trim_dense_threshold();
}
                                    ...
                                    UINT32* R_io)
{
    ...
    static UINT32 bucket_starts[USER_CLUSTERS + 1];
    ...
    // --- (1) 이번 레벨 후보 개수 집계: "절반 이상 찬" 세그먼트만 카운트 ---
    for (UINT32 seg = 0; seg < SEGMENT_COUNT; ++seg) {
        if (g_trim_seg_count[seg] == level_cnt && is_dense_segment(seg)) {
    ...
    if (total_candidates == 0u) return 0u;  // 이번 레벨에는 조건 만족 후보 없음 → 스킵
    ...
    // --- (2) 후보 배열 구성 (여기서도 50% 미만은 아예 안 넣음) ---
    for (UINT32 seg = 0; seg < SEGMENT_COUNT; ++seg) {
        if (g_trim_seg_count[seg] == level_cnt && is_dense_segment(seg)) {
            UINT32 cid = get_cluster_id_from_seg(seg);
            if (cid >= C_eff) continue;
            candidates[bucket_cursor[cid]++] = seg; // seg index 오름차순 축적
        }
    }
    ...
    // 남은 슬롯은 rem 큰 쪽부터 1씩 보정(버킷 cap 고려)
    while (given < R) {
        UINT32 best_c = C_eff;
        UINT64 best_r = 0;
        for (UINT32 c = 0; c < C_eff; ++c) {
        ...
        if (best_c == C_eff) break; // 더 늘릴 곳 없음
        alloc[best_c] += 1;
        given += 1;
...
UINT32 FindTopKTrimSegments(UINT32 k)
{
    ...
    // --- (A) max_cnt 계산: "절반 이상 찬 세그먼트"만 고려 ---
    UINT16 max_cnt = 0;
    for (UINT32 i = 0; i < SEGMENT_COUNT; ++i) {
        if (!is_dense_segment(i)) continue;                  // 50% 미만 → 무시
        if (g_trim_seg_count[i] > max_cnt) max_cnt = g_trim_seg_count[i];
    }
    if (max_cnt == 0u) return 0u; // dense 세그먼트가 하나도 없음 → 바로 종료
    ...
    for (UINT32 i = 0; i < out_n; ++i) {
        UINT32 seg = chosen_idx[i];
        // 안정성: 경합으로 밀렸다면 스킵(거의 없겠지만)
        if (!is_dense_segment(seg)) continue;

        const UINT32 meta_base_lpn = seg * 1024u;
